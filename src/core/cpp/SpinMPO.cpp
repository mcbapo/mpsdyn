...
void SpinMPO::getSz2MPO(int L,int spindim,MPO& Sz2){
  ...
  Cl.multiplyRight(Z);
  Cr.multiplyRight(Z);
  C.reshape(Indices(D,D,d,d));C.permute(Indices(3,1,4,2));
  Cl.reshape(Indices(1,D,d,d));Cl.permute(Indices(3,1,4,2));
  Cr.reshape(Indices(D,1,d,d));Cr.permute(Indices(3,1,4,2));
...
void SpinMPO::getMomentumDistributionMPO(int L,int dim,int k,MPO& Nkmpo,
  ...
  if(!prepared){
    ...
    mwArray Nop(idOp);Nop.setElement(ZERO_c,Indices(1,1));
    ...
    Cl.multiplyRight(Z);
    Cr.multiplyRight(Z);
    C.reshape(Indices(D,D,dim,dim));C.permute(Indices(3,1,4,2));
    Cl.reshape(Indices(1,D,dim,dim));Cl.permute(Indices(3,1,4,2));
    Cr.reshape(Indices(D,1,dim,dim));Cr.permute(Indices(3,1,4,2));
    // Assign the operators to the MPO
...
void SpinMPO::getSkMPO(int N,int dim,int z,MPO& Skmpo,bool prepared){
  ...
  if(!prepared){
    ...
    mwArray term=C*Z;
    term.reshape(Indices(D,D,dim,dim));term.permute(Indices(3,1,4,2));
    mwArray termL=Cl*Z;
    termL.reshape(Indices(1,D,dim,dim));termL.permute(Indices(3,1,4,2));
    mwArray termR=Cr*Z;
    termR.reshape(Indices(D,1,dim,dim));termR.permute(Indices(3,1,4,2));
...
void SpinMPO::getSingleBodyMPO(int L,const mwArray& idOp,
  ...
  Cl.setElement(ONE_c,Indices(0,D-1,1));
  int signR=1;
  if(staggered && (L-1)%2!=0 ) signR=-1;
  Cr.setElement(signR*ONE_c,Indices(0,0,1));
  // if staggered, I set a C for odd sites
  mwArray Codd(C);
  if(staggered) Codd.setElement(-1*ONE_c,Indices(0,D-1,1)); // only this element changes sign
  C.reshape(Indices(D*D,2));Codd.reshape(Indices(D*D,2));
  Cl.reshape(Indices(D,2));  Cr.reshape(Indices(D,2));
  mwArray term=C*Z;
  ...
  if(!staggered)
    ...
    termOdd=Codd*Z;
    termOdd.reshape(Indices(D,D,d,d));termOdd.permute(Indices(3,1,4,2));
  }
  mwArray termL=Cl*Z;
  ...
  if(!staggered){
	...
	for(int k=3;k<L-1;k+=2){
	  Smpo.setOp(k,&Smpo.getOp(1),false);
	  if(k+1<L-1) Smpo.setOp(k+1,&Smpo.getOp(2),false);
	}
      }
    }
...
void SpinMPO::getProjectorTotalSPair(int L,int spindim,MPO& PS1,bool even){
  ...
  mwArray basR(basL);basR.permute(Indices(1,4,3,2));
  ...
  int k=firstL;
  bool usedL=false;bool usedR=false;
  ...
  if(k==L-1){ // after the last OR was placed in L-2, 
...
void SpinMPO::getModulatedSingleBodyMPO(int N,const mwArray& idOp,
  ...
  for(int l=0;l<N;l++){
    ...
    if(obc) factorQ=sin(l*q*M_PIl)*ONE_c;
    ...
    mwArray term=C*Z;
    term.reshape(Indices(Dl,Dr,d,d));term.permute(Indices(3,1,4,2));
