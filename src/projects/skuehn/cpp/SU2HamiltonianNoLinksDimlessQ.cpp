/**
   \file SU2HamiltonianNoLinksDimlessQ.cpp
...
//Constructor
SU2HamiltonianNoLinksDimlessQ::SU2HamiltonianNoLinksDimlessQ(int N_,double epsilon_,double mg,int dlink_, double lambda_, double sz_penalty_, double charge_penalty_)
{
  ///Number of fermions
  this->N_fermions=N_;
  //Hopping constant
...
//Destructor
SU2HamiltonianNoLinksDimlessQ::~SU2HamiltonianNoLinksDimlessQ(){}

//Build the Hamiltonian MPO
void SU2HamiltonianNoLinksDimlessQ::initHamiltonian()
{
  //Compared to the model without charge penalty, my bond dimension is increased by one
  int Dbond=this->d_link+16+1+1+1,Dl,Dr;
  
  cout << "Building dimensionless Hamiltonian MPO containing charge penalty with" << endl
  ...
  for(int d=0; d<this->d_fermi; d++)
  {
    ...
    else if(d==2)
    {
      ...
      electric_tensor.setElement(1.0/2.0*(1.0/2.0+1.0)*ONE_c,Indices(d,0,d,Dbond-1));
    }
    //Mass tensor
...
#ifdef MATOUT
  hamil.exportForMatlab("SU2HamiltonianNoLinksDimlessQ.m");
#endif
}
