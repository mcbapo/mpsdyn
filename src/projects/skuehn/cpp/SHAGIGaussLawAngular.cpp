#include "SHAGIGaussLawAngular.h"
#include "Indices.h"
...
SHAGIGaussLawAngular::SHAGIGaussLawAngular(int N_, int d_,int D_,double mu_,double x_,double e_,double lambda_,unsigned int exp_order_):hamil(2*N_-1),charge(2*N_-1)
{
  N_fermions = N_;
  ...
  cout << "Constructing NON gauge invariant Hamiltonian up to the desired order with " << endl
...
//Destructor
SHAGIGaussLawAngular::~SHAGIGaussLawAngular(){
    hamil.clear();
    charge.clear();    
}

void SHAGIGaussLawAngular::initOperators()
{
  ...
  for(int i=0; i<this->d_bose; ++i)
  {
    x = -1.0*(this->d_bose-1)/2.0 + (double) i;
    for(int j=0; j<this->d_bose; ++j)
    {
      y = -1.0*(this->d_bose-1)/2.0 + (double) j;
      tmp=ZERO_c;
      for(int k=0; k<this->d_bose; ++k)
      {
	k_val= -1.0*(this->d_bose-1)/2.0 + (double) k;
	//cout << "x=" << x << " y=" << y << " k=" << k_val << endl;
	tmp = tmp + 2.0*M_PI/((double) this->d_bose)*k_val*exp(-2.0*I_c*M_PI/((double) this->d_bose)*k_val*(x-y));
      }
  ...
  //Approximate the exponential up to the desired order    
  mwArray TempOp1 = identityMatrix(this->d_bose);
  mwArray TempOp2 = identityMatrix(this->d_bose);
  double faculty = 1;
  for(int i=0; i<=this->exp_order; i++)
  {
    Op1 = Op1 + 1.0/faculty*TempOp1;
    Op2 = Op2 + 1.0/faculty*TempOp2;    
    
    TempOp1 = TempOp1*(I_c*Theta);
    TempOp2 = TempOp2*(-1.0*I_c*Theta);
...
//   cout << "Op2 = " << Op2 << endl;
  ...
  //cout << hamil << endl;
  //hamil.exportForMatlab("HSHAGIGaussLawAngular.m");
}// initOperators

void SHAGIGaussLawAngular::initChargeoperator()
{
...
void SHAGIGaussLawAngular::constructLopMPO(MPO &mpo, int n)
{
  ...
  if((n%2)!=1)
  {
    cerr << "Error in SHAGIGaussLawAngular::constructLopMPO(MPO &mpo, int n): Index " << n << " is not a valid bosonic site" << endl;
    exit(1);
  }
...
void SHAGIGaussLawAngular::constructGaussMPO(MPO &mpo, int n)
{
  ...
  if((n%2)==1)
  {
    cerr << "Error in SHAGIGaussLawAngular::constructGaussMPO(MPO &mpo, int n): Index " << n << " is not a valid fermionic site" << endl;
    exit(1);
  }
...
void SHAGIGaussLawAngular::constructsigmazMPO(MPO &mpo, int n)
{
  ...
  if((n%2)==1)
  {
    cerr << "Error in SHAGIGaussLawAngular::constructsigmazMPO(MPO &mpo, int n): Index " << n << " is not a valid fermionic site" << endl;
    exit(1);
  }
...
void SHAGIGaussLawAngular::constructCondensateMPO(MPO &mpo)
{
...
void SHAGIGaussLawAngular::constructPenaltyMPO(MPO &mpo)
{
