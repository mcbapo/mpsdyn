...
/** 
    ...
    add the \f$ e^{-s} \sqrt{c(1-c)}
    \sigma_x^{[i+1]}-c-(1-2c)n^{[i+1]})\f$ operator
*/

class StochasticHamiltonian:
public Hamiltonian{
protected:
  int L; // chain length
 ...
 public:
  StochasticHamiltonian(int L,double s,double offset=0.,bool sectorOne=true,double c=0.5,bool pbc=0,double noZero=0.);

  ~StochasticHamiltonian();
