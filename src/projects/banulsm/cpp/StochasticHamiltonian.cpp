/**
   \file StochasticHamiltonian.cpp
...
StochasticHamiltonian::StochasticHamiltonian(int L_,double s_,double offset_,bool sec1,double c_,bool pbc_,double noZero_):d(2),c(c_),L(L_),hamil(L),s(s_),offset(offset_),sectorOne(sec1),pbc(pbc_),noZeroPenalty(noZero_){
  initZ();
  if(pbc)
    initHMPOpbc();
  else
...
StochasticHamiltonian::~StochasticHamiltonian(){};

void StochasticHamiltonian::initZ(){
  ...
  if(c==0.5&&s<=-1111){
    cout<<"Special case, s->-infty: no c factor included!"<<endl;
    operB=sig1;
  }
  ...
  for(int i1=0;i1<d;i1++)
    for(int i2=0;i2<d;i2++){
      ...
      Z.setElement(operB.getElement(Indices(i1,i2)),Indices(2,i1,i2));
    }
  Z.reshape(Indices(3,d*d));
}

void StochasticHamiltonian::initHMPO(){
  int D=3;
  for(int k=0;k<L;k++){
    ...
    if(k>1&&k<L-1){
      ...
      //cout<<"Creating new Operator for site "<<k<<" of StochasticH MPO"<<endl;
      int Dl=D; int Dr=D;
      if(k==0) Dl=1;
      if(k==L-1) Dr=1;
	...
	// Second of the pair
	//	C.setElement(-exp(-s)*ONE_c,Indices(1,Dr-1,2));
	C.setElement(-ONE_c,Indices(1,Dr-1,2));
      }
      else{
	if(sectorOne){ // a second operator on the first site
	  C.setElement(-ONE_c,Indices(0,Dr-1,2));	
	}
      }
      if(offset!=0)
...
void StochasticHamiltonian::initHMPOpbc(){
  //  cout<<"Initializig PBC East model with s="<<s<<", c="<<c<<endl;
  //putForMatlab(cout,Z,"Z");
  int D=4;int nOp=3;
  if(noZeroPenalty!=0){
    D=5;nOp=4;
    // basic spin operators appearing
    ...
    mwArray operN0=sig0-operN;
    Z=mwArray(Indices(nOp,d,d));Z.fillWithZero();
    for(int i1=0;i1<d;i1++)
      for(int i2=0;i2<d;i2++){
	Z.setElement(sig0.getElement(Indices(i1,i2)),Indices(0,i1,i2));
	Z.setElement(operN.getElement(Indices(i1,i2)),Indices(1,i1,i2));
	Z.setElement(operB.getElement(Indices(i1,i2)),Indices(2,i1,i2));
	Z.setElement(operN0.getElement(Indices(i1,i2)),Indices(3,i1,i2));
      }
  ...
  for(int k=0;k<L;k++){
    ...
    if(k>1&&k<L-1){
      ...
      //  cout<<"Creating new Operator for site "<<k<<" of PBC StochasticH MPO"<<endl;
      int Dl=D; int Dr=D;
      if(k==0) Dl=1;
      if(k==L-1) Dr=1;
      mwArray C(Indices(Dl,Dr,nOp));C.fillWithZero();
      // Set elements
      if(k!=L-1){
	C.setElement(ONE_c,Indices(0,0,0)); // nothing yet
	// First of the pair
	...
	if(pbc) 
	  C.setElement(ONE_c,Indices(2,Dr-1,1)); // first of the last pair
      }
      if(k!=0){ 
	// After everything
	C.setElement(ONE_c,Indices(Dl-1,Dr-1,0));
	...
	if(pbc){
	  C.setElement(-ONE_c,Indices(0,2,2)); // second of the last pair in PBC case
	}
	else{
	  if(sectorOne){ // a second operator on the first site
	    C.setElement(-ONE_c,Indices(0,Dr-1,2));	
...
void StochasticHamiltonian::getExponentialMPOeven(MPO& expHe,complex_t delta) const {
 getExponentialMPO(expHe,delta,true);
}
void StochasticHamiltonian::getExponentialMPOodd(MPO& expHo,complex_t delta) const {
 getExponentialMPO(expHo,delta,false);
}

void StochasticHamiltonian::getTwoBodyTermExponential(mwArray& Ol,mwArray& Or,complex_t delta,
						      int pos) const {
  mwArray H12;
...
void StochasticHamiltonian::getTwoBodyTermExponential(mwArray& Ol,mwArray& Or,
...
void StochasticHamiltonian::getExponentialMPO(MPO& expH,complex_t delta,bool even) const {
  mwArray Ol,Or;
  expH.initLength(L);
  int kfirst=even?0:1;
...
void StochasticHamiltonian::getCommutatorMPO(MPO& mpo){
...
void StochasticHamiltonian::initZdoub(mwArray& Z){
  cout<<"Not ready yet!"<<endl;
  exit(1);
  // // Basic pieces
...
void StochasticHamiltonian::constructOperatorProduct(mwArray& result,const mwArray& opA,
						const mwArray& opB) const{
#ifdef CHECKDIMS
  if(opA.getDimensions()!=opB.getDimensions()){
    cout<<"Error in StochasticHamiltonian::constructOperatorProduct for"
...
void StochasticHamiltonian::computeTwoBodyTerm(mwArray& result,int pos) const{
  if(pos>=L-1){
    cout<<"ERROR: StochasticHamiltonianHamiltonian::computeTwoBodyTerm for site "<<pos<<" when L="<<L<<endl;
    exit(212);
  }
  static mwArray NX,NI,BI,NN;
  ...
  if(firstCall){
    ...
    if(sectorOne)
      constructOperatorProduct(BI,-exp(-s)*sqrt(c*(1-c))*sig1+c*sig0+(1-2*c)*operN,sig0);
  }
  result=-exp(-s)*sqrt(c*(1-c))*NX+c*NI+(1-2*c)*NN;
  if(pos==0&&sectorOne){
    result=result+BI;
  }
}
