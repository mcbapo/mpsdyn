/**
   \file LimitStochasticHamiltonian.cpp
...
LimitStochasticHamiltonian::LimitStochasticHamiltonian(int L_,double offset_,bool sec1):d(2),L(L_),hamil(L),offset(offset_),sectorOne(sec1){
  initZ();
  initHMPO();
}

LimitStochasticHamiltonian::~LimitStochasticHamiltonian(){};

void LimitStochasticHamiltonian::initZ(){
...
void LimitStochasticHamiltonian::initHMPO(){
  int D=3;
  for(int k=0;k<L;k++){
    ...
    if(k>1&&k<L-1){
      ...
      //cout<<"Creating new Operator for site "<<k<<" of LimitStochasticH MPO"<<endl;
      int Dl=D; int Dr=D;
      if(k==0) Dl=1;
      if(k==L-1) Dr=1;
...
void LimitStochasticHamiltonian::getExponentialMPOeven(MPO& expHe,complex_t delta) const {
  getExponentialMPO(expHe,delta,true);
}
void LimitStochasticHamiltonian::getExponentialMPOodd(MPO& expHo,complex_t delta) const {
  getExponentialMPO(expHo,delta,false);
}

void LimitStochasticHamiltonian::getTwoBodyTermExponential(mwArray& Ol,mwArray& Or,complex_t delta,
							   int pos) const {
  mwArray H12;
...
void LimitStochasticHamiltonian::getTwoBodyTermExponential(mwArray& Ol,mwArray& Or,
...
void LimitStochasticHamiltonian::getExponentialMPO(MPO& expH,complex_t delta,bool even) const {
  mwArray Ol,Or;
  expH.initLength(L);
  int kfirst=even?0:1;
...
void LimitStochasticHamiltonian::getCommutatorMPO(MPO& mpo){
...
void LimitStochasticHamiltonian::initZdoub(mwArray& Z){
  cout<<"Not ready yet!"<<endl;
  exit(1);
  // // Basic pieces
...
void LimitStochasticHamiltonian::constructOperatorProduct(mwArray& result,const mwArray& opA,
						const mwArray& opB) const{
#ifdef CHECKDIMS
  if(opA.getDimensions()!=opB.getDimensions()){
    cout<<"Error in LimitStochasticHamiltonian::constructOperatorProduct for"
...
void LimitStochasticHamiltonian::computeTwoBodyTerm(mwArray& result,int pos) const{
  if(pos>=L-1){
    cout<<"ERROR: LimitStochasticHamiltonianHamiltonian::computeTwoBodyTerm for site "<<pos<<" when L="<<L<<endl;
    exit(212);
  }
  static mwArray NX,XI;
  ...
  if(firstCall){
    ...
    if(sectorOne)
      constructOperatorProduct(XI,sig1,sig0);
  }
  result=-ONE_c*NX;
  if(pos==0&&sectorOne){
    result=result+XI;
