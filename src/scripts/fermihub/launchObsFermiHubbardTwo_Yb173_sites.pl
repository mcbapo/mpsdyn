...
my $scriptFile="lanzaObs2_Yb173.sh";

my $EXEC="/u/banulsm/NewSVN/mpsdyn/src/bin/fH2obs2_v0";
...
#my $D=$MAXD; #just the largest
my $D=100; $MAXD=100; #only this one
...
foreach my $Ngtarget (@Ngs){
	...
	my $outfileSite="N${Ntot}Ng${Ngtarget}V${V}sites";

	my $lastDcomp=checkLastLine("${outdir}/${outfileSite}");
	if($lastDcomp<$MAXD || !$app){
	    if($lastDcomp>0){
	    ...
	    my $args="${configfile} -output=${outdir}/${outfile} -outputSite=${outdir}/${outfileSite} -mpsdir=${dirMPSs} -jobsdir=${jobsdir} -tol=${tol} -L=${L} -freqSv=${freqSv}";
