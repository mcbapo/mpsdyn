...
my $scriptFile="lanzaObs_Yb171.sh";

my $EXEC="/u/banulsm/NewSVN/mpsdyn/src/bin/fH2obs_v0";

my $basicDir="/ptmp/mpq/banulsm/fermihubbard2/all/cold_atoms/Yb171";
...
#my @Ngs=(1); $MAXD=300; #trick for all of them to be recomputed
###### refine
#my $L=20;
#my @Ngs=(17);
#my @Nes=(17);
#my $scriptFile="refineObs_Yb171.sh";
###### refine
...
foreach my $Ngtarget (@Ngs){
	...
	# 30 min should be enough, give 1 hr
	my $TIME="-t 1:00:00 ";
	...
	if($lastDcomp<$MAXD){
	    if($lastDcomp<220){
		print "Last D for Ng${Ngtarget}.Ne${Netarget} ${lastDcomp}\n";
	    }
