...
# Now Q represents a factor, such that q actually is this times 1/N (should it be 1/(N+1) for OBC?)
my @Qs=(1.,2.,3.,4.,5.);
...
#	else{
		...
		my $args=" -L=${N} -D=${D0} -s=${s} -c=${c} -q=${valq} -nlevel=${nlev} -outputfile=${outfile} -mpsdir=${mpsDir} -firstN=1 -jobsdir=${jobsdir} -maxD=${maxD} -incrD=${incrD} -append=1 ";
