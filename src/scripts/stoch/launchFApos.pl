...
my $ignoretmp=" -ignoretmp=1 "; # to ignore previous results of same D
...
# So I also use this one for c=0.3
my $smin = 1;
my $smax = 100;
my $sstep = 1;
my @sValues3 = map { ($smax-$sstep*$_)/1000 } 0..($smax-$smin)/$sstep;
...
## LOOp over s values!
	foreach $s (@sValues){
		...
		my $args=" -L=${N} -D=${D0} -sL=${s} -c=${c} -nlevel=${nlev} -outputfile=${outfile} -polfile=${polfile} -mpsdir=${mpsDir} -initmpsdir=${initmpsDir} -tol=${mytol} -tmpSavingFreq=${saveFreq} -firstN=1 ${reorder} -noise=${noise}  ${refine} -jobsdir=${jobsdir} -maxD=${maxD} -incrD=${incrD} -knr=${knr} -eigtol=${eigtol} -penaltyZero=$penZero -pbc=${pbc} ${ignoretmp} ";
