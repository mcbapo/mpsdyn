...
$basicdir="/ptmp/mpq/banulsm/TransverseLC/Ising/Lmax${Lmax}";
$basictmpdir="/ptmp/mpq/banulsm/TransverseLC/IsingTmp/Lmax${Lmax}";
...
#	$savingfreq=50;
    ...
    $NAME="tlc${jobStr}${initStName}_g${glab}h${hlab}_dt${dt}_D${Dcut}_L${Lmax}";
    my $filename="init${initSt}_L${L}J${Jlab}g${glab}h${hlab}dt${dtlab}_D${Dcut}";
    my $outfile="${outputdir}/${filename}";
    my $args=" ${configfile} -J=${JP} -g=${gP} -h=${hP} -M=${M} -outputfile=${outfile} -delta=${dt} -D=${Dcut} -savingfreq=${savingfreq} -initSt=${initSt} -rate=${rate} -tmpdir=${tmpdir} -append=${append} -Lmax=${Lmax} ";
