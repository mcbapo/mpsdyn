...
foreach $Mlog (@Mlogs){
	...
	foreach $g (@Gs){
		...
		if($D<=$exactD){
		    ...
		    print MYFILE "${MSUB} ${par} -N ${JOBNAME} -- ${EXEC} ${configFile} -L=${N} -J=${J} -g=${g} -h=${h} -D=$D -Nbins=${nrBins} -f0=${f0} -outputFile=${outhistoFile} -outputFileSummary=${outdataFile} -maxCnt=${M} -maxCntF=${maxF} -BPcrit=1 \n";
