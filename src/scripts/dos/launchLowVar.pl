...
my $jobsdir="/afs/ipp/u/banulsm/jobsVar/";
...
foreach $paramSet (@hamilPars){
	...
	my $args=" -L=${N} ${paramSet} -D=${Dmin} -mpsdir=${mpsdir} -tol=${tol} -penH=${penH} -Dmin=${Dmin} -Dmax=${Dmax} -nrStates=${nrStates} -jobsdir=${jobsdir} -tol=${tol} ";
	...
	print MYFILE "${MSUB} ${par} -N ${JOBNAME} -- ${EXEC} ${configFile} ${args} -greedy=1 \n";
