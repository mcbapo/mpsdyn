...
# Script to prepare a series of jobs using the null projector program

# Name of the script file to be created
my $scriptFile="lanzaDiagPW.sh";

my $EXEC="./projWeak";

my $configFile="../config/dissipXY.conf"; #ignored!

my $basicDir="/ptmp/mpq/banulsm/diagonal/projW";
...
foreach $paramSet (@hamilPars){
	...
	foreach $N (@Ns){
	    ...
	    my $JOBNAME="projW.N${N}.D${D}.${cnt}";
	    # Check that the job is not yet running!!
	    #my $MEM=int(((2*$N+1)*$Nb*$D*$D*4+(2*$N+1)*2*$Nb*$Nb*$Nb*$Nb)*16*1E-6)*2+200;
	    my $args=" -L=${N} ${paramSet} -D=${D} -output=${outfile} -mpsdir=${mpsdir} -mpsfile=${mpsfile} -M=${Nsteps} -rate=${rate} -saveFreq=${saveRate} -initSt=${initSt} ";
