...
my $scriptFile="lanzaDiagEP.sh";

my $EXEC="./diagEP";

my $configFile="../config/dissipXY.conf"; #ignored!

my $basicDir="/ptmp/mpq/banulsm/diagonal/diagEP";
...
foreach $paramSet (@hamilPars){
	...
	foreach $N (@Ns){
	    ...
	    my $args=" -L=${N} ${paramSet} -D=${D} -outputfile=${outfile} -mpsdir=${mpsdir} -mpsfile=${mpsfile} -M=${Nsteps} -rate=${rate} -saveFreq=${saveRate} -initSt=${initSt} ";
