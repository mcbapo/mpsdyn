...
my @Ns=(50,100,200,300);
my @Ds=(100,200,300);
my @pars=("-P 10","-P 12","-P 14","-P 20"); # use parallel environment
...
foreach $paramSet (@hamilPars){
	...
	foreach $N (@Ns){
	    ...
	    my $args=" -L=${N} ${paramSet} -D=${D} -output=${outfile} -mpsdir=${mpsdir} -mpsfile=${mpsfile} -x=${deltaX} -dt=${dt} -M=${Nsteps} -rate=${rate} -saveFreq=${saveRate} -initSt=${initSt} -checkRDM=1 -Lcmax=$Lcmax -average=$avrg -Lb=$Lb -seed=$randSeed ";
