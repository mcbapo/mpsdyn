...
my $scriptFile="analyzeChebyDelta.sh";
...
sub isRunning{
    ...
    print "Job $jobname does not exist: $result \n";
    return 0;
};
...
#    @Ns=(20,40,60,50,80,100);@Ds=(200,300,400,500,1000);
    ...
    @pars=("-P 10","-P 10","-P 10","-P 10");
    @mems=("-mem 30G","-mem 30G","-mem 30G","-mem 30G");
...
foreach $paramSet (@hamilPars){
	...
	foreach $N (@Ns){
	    ...
	    my @Ms=(2*$N,int(2*sqrt($N)),5*$N,10*$N,int(5*sqrt($N)),int(10*sqrt($N)),int($N*log($N)),int(0.1*($N**2)),20*$N,50*$N); # just for Ising!!!
	    #my @Ms=0;
	    ...
	    my $locmem=$mem;
	    foreach $M (@Ms){
		if(D==1000){$locpar=" -P 40 ";$locmem=" -mem 100G "}
		else{$locpar=" -P 20 ";$locmem=" -mem 60G ";}
		...
		my $JOBNAME="chD.analyze.${modelStr}.N${N}.D${D}.M${M}.par${cnt}";
		...
		if(! $alreadyRunning ){
			...
			my $JOBdata="chD.Sred.${modelStr}.N${N}.D${D}.M${M}.par${cnt}";
			my $runningData=isRunning($JOBdata);
			if(! $runningData){
			    my $args=" -L=${N} ${paramSet} -D=${D} -delta=${delta} -initSt=${initSt} -outputSblock=${outfileSblock} -outputCorr=${outfileCorr}  -M=${M} -initSt=${initSt} -average=$avrg -mpsdir=${mpsdir} -mpsfileroot=${mpsfileroot} -append=${app} ";
