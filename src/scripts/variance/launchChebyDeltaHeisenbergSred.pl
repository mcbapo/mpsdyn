...
my $scriptFile="lanzaChebyDeltaHeis.sh";
...
#	@Ns=(20,40,60,50,80,100);@Ds=(400,500,1000);
	...
	@pars=("-P 20","-P 20","-P 20","-P 40");
	@mems=(" -mem 60G "," -mem 60G "," -mem 60G "," -mem 120G ") ; # -mem ${MEM}G 
...
foreach $paramSet (@hamilPars){
	...
	my $mem=$mems[$cntD];
	foreach $N (@Ns){
	    my @Ms=(2*$N,int(2*sqrt($N)),5*$N,10*$N,int(5*sqrt($N)),int(10*sqrt($N)),20*$N);
	    #my @Ms=(int(0.1*($N**2)),int($N*log($N)));
	    #my @Ms=(int($N*log($N)));
	    my $locpar=$par;
	    my $locmem=$mem;
	    foreach $M (@Ms){
		...
		my $JOBNAME="chD.${modelStr}.N${N}.D${D}.M${M}.par$labParH[${cnt}]";
		...
		if(! $alreadyRunning ){
			...
			print "Not launching $JOBNAME because it already reached M=$lastM and file $finalMPSName exists\n";
		    }
		}
		else{
