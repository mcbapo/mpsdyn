...
int main(int argc,const char* argv[]){
  ...
  if(!app||!file_exists(outfname.data())){
    ...
    *out<<"% Thermal Ising with J="<<J<<", g="<<g<<" h="<<h<<", L="<<N<<endl;
    ...
    //*out<<"% N\t g\t h\t D\t delta\t nrsteps\t beta\t <H>\t <h(L/2)>\t <sigx(L/2)>\t <sigy(L/2)>\t <sigz(L/2)>\t "<<endl; 
    *out<<"% N\t J\t g\t h\t D\t delta\t nrsteps\t beta\t T\t Energy/N\t <Sx>/N\t"
	<<" <Sz>/N\t <sigx(N/2)\t <sigz(N/2)>\t log(Z)"<<endl;
  ...
  if(tol>0) contractor.setConvTol(tol);
  int d0=2;
  MPS thS(N,1,d0*d0);
  ...
  mwArray opX,opZ;
  mwArray sig0=identityMatrix(d0);//identity
  complex_t datax[]={ZERO_c,ONE_c,ONE_c,ZERO_c};
  mwArray sigX(Indices(d0,d0),datax);//sigmax
  complex_t dataz[]={ONE_c,ZERO_c,ZERO_c,-ONE_c};
  mwArray sigZ(Indices(d0,d0),dataz);//sigmaz
