...
int main(int argc,const char* argv[]){
  ...
  vector<vector<double> > coeffs(maxN*(maxN+1)/2,vector<double>()); // the modulation for each of them
  for(int n=1;n<=maxN;n++){ // Repeat for n-th power of the
			       // Hamiltonian
    for(int r=0;r<n;r++){
      int posnr=r+(n-1)*n/2;
      vector<MPS*>& opBasis=opBasis_[posnr];
      vector<double>& coeffN=coeffs[posnr];
      coeffN.clear();
      prepareOperatorsHnrange(n,r+1,H12,M,opBasis,coeffN);
      //**** All ops appearing in Hn with range n+1 are now ordered in
      //**** opBasis_[n-1]
      //prepareCoefficientsrange(n,r,M,coeffN);
      cout<<"Prepared coefficients of term n="<<n<<", r="<<r+1<<": "<<coeffN<<endl;
    }
  }
  MPS mpsId(M,1,d*d);
  {  
