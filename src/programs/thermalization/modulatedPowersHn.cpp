...
int main(int argc,const char* argv[]){
  ...
  vector<vector<MPS*> > opBasis_(maxN,vector<MPS*>());
  vector<vector<double> > coeffs(maxN,vector<double>()); // the modulation foe each of them
  for(int n=1;n<=maxN;n++){ // Repeat for n-th power of the
			       // Hamiltonian
    vector<MPS*>& opBasis=opBasis_[n-1];
    vector<double>& coeffN=coeffs[n-1];
    prepareOperatorsHn(n,H12,M,opBasis);
    //**** All ops appearing in Hn with range n+1 are now ordered in
    //**** opBasis_[n-1]
    prepareCoefficients(n,M,coeffN);
    //cout<<"Prepared coefficients of term "<<n<<": "<<coeffN<<endl;
  }
  MPS mpsId(M,1,d*d);
  {  
    MPO idMPO(M);
  ...
  // pair n,m
  mwArray Heff_nm(Indices(maxN,maxN));
  mwArray Neff_nm(Indices(maxN,maxN));
  for(int n=1;n<=maxN;n++){ // Repeat for n-th power of the
    vector<MPS*>& opBasisN=opBasis_[n-1];
    vector<double>& coeffN=coeffs[n-1];
    mwArray vecN(coeffN,false);
    for(int m=n;m<=maxN;m++){
      vector<MPS*>& opBasisM=opBasis_[m-1];
      vector<double>& coeffM=coeffs[m-1];
	...
	for(int j=0;j<mTerms;j++){
      ...
      Heff_nm.setElement(valH.getElement(0),Indices(n-1,m-1));
      Neff_nm.setElement(valN.getElement(0),Indices(n-1,m-1));
      if(m!=n){
	Heff_nm.setElement(conjugate(valH.getElement(0)),Indices(m-1,n-1));
	Neff_nm.setElement(conjugate(valN.getElement(0)),Indices(m-1,n-1));
      }
    }
  }
  ...
  for(int in=0;in<maxN;in++){ // for each value
    ...
    effN.resize(Indices(in+1,in+1));
    effH2.resize(Indices(in+1,in+1));
