...
/** 
    ...
    by bestModulatedOp and compute the commutator for a large range of
    sies, to check the scaling.
    The modulating function can be changed by parameter -modulation=mode
    mode: 0=cosine (default); 1=Gaussian; 2=const
*/
...
int main(int argc,const char* argv[]){
  ...
  bool done=false;
  string aLine;
  vector<complex_t> coefs(1,ZERO_c); // first one is zero (Identity)
  int nrTerms=pow(4,range); // basis elements
  double norm=0; // max absolute value of coeffs
  complex_t phase=ONE_c; //possible overall phase of the vector of coefficients
  while(input.good()&&!done){
    ...
    str>>_M>>_n;
    cout<<"Read line M="<<_M<<", range="<<_n<<endl;
    if(_n==range){ // read the coeffs
      cout<<"This LINE I keep "<<endl;
      // But I need to discard the lambda value!
      double lamb; str>>lamb;
      long double _re,_im;
      for(int k=0;k<nrTerms;k++){
	str>>_re>>_im;
	complex_t component={_re,_im};
	if(abs(component)>norm){
	  norm=abs(component);
	  phase=component/abs(component);
	}
	coefs.push_back(component);
      }
      done=true;
    }
  }
