...
*/
const string mpsfilename(int L,int N,int D,double t,double U,int Ntot,
...
int findInitFile(string& str,int L,int N,int D,double t,double U,int Ntot,
...
int main(int argc,const char* argv[]){
  ...
  double penN=props.getDoubleProperty("penaltyN");
  ...
  if(!app||!file_exists(outfname)){
    ...
    *out<<"% t="<<t<<", U="<<U<<", Ntotal="<<Ntot<<" penalty="<<penN<<endl;
  ...
  BoseHubbardHamiltonian hBH(L,N,t,U,Ntot,penN);
 ...
 // Observable(s) to compute
  ...
  while(oldMPS&&lval<=nlev-1){
    const string filename=mpsfilename(L,N,D,t,U,Ntot,lval,mpsdir);
  ...
  while(lval<=nlev-1){
    ...
    if(initmpsdir.size()>0){
      string filename;
      int Dinit=findInitFile(filename,L,N,D,t,U,Ntot,lval,initmpsdir);
      if(Dinit>0){
...
int findInitFile(string& str,int L,int N,int D,double t,double U,int Ntot,
  ...
  for(int myD=D-1;myD>0;myD--){
    ...
    str=mpsfilename(L,N,myD,t,U,Ntot,level,initmpsdir);
    if(file_exists(str)){
...
const string mpsfilename(int L,int N,int D,double t,double U,int Ntot,
			 int level,const string mpsdir){
  stringstream s;
  s<<mpsdir<<"/MPS_L"<<L<<"_t"<<t<<"_U"<<U<<"_N"<<Ntot<<"_nmax"<<N<<"_D"<<D<<"_l"<<level<<".dat";  
