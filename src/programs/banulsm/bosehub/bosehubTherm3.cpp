...
/** bosehubTherm3 tries to find the thermal state of the Bose-Hubbard
    Hamiltonian \ref <BoseHubbardHamiltonian> using purification 
    where the dimension of the auxiliary sites is smaller than the
    physical. Then applies imaginary time evolution, splitting the 
    auxiliary and true sites.

    Receives arguments:
    \param <L> (int) length of the chain
    \param <N> (int) cutoff for the (physical) occupation number
    \param <Na> (int) cutoff for the (ancilla) occupation number
...
int main(int argc,const char* argv[]){
  ...
  *out<<setprecision(10);
  ...
  mwArray Aodd(shrt::Indices(da,D,D));
  int dmin=min(d0,da);
  double val=1./sqrt(sqrt(dmin));
  ...
  for(int k=0;k<L;k++){
    ...
    if(k==L-1){
      ...
      lastSite.resize(shrt::Indices(da,D,1));
  ...
  double lambda=0.;
  MPS gS(2*L,D,dims);gS.setRandomState();
