...
int main(int argc,const char* argv[]){
  ...
  bool truncH=props.getIntProperty("truncH")==1; // Hatings truncation? default is std
 ...
 // The operator I want to compute on L sites (for now, just a sigma_x in the center)
  ...
  // putForMatlab(cout,edgeUL,"edgeUL");
  // putForMatlab(cout,edgeURb,"edgeURb");
  // putForMatlab(cout,boundaryUL,"boundaryUL");
  // putForMatlab(cout,boundaryUR,"boundaryUR");
  ...
  //  if(cnt==0){
    ...
    if(energyCs){
      ...
      computeDistantCorrelatorsTwoBodyOpt(mpsB,midU,edgeUL,edgeULb,edgeUR,edgeURb,
					  //computeDistantCorrelatorsTwoBody(mpsB,midU,edgeUL,edgeULb,edgeUR,edgeURb,
				       cnt,delta,outfname,
				       Dcut,M,eDens,eDens,labelE);
    }
    //}
  ...
  // 3) contract expectation values
    ...
    else{//Hastings' truncation STILL TESTING!!!
      for(int nextra=0;nextra<toadd;nextra++){
	mpsB.insertSite(0,reshape(mwArray(ONE_c),Indices(1,1,1)),true,false); // check dims but do not normalize
	mpsB.insertSite(mpsB.getLength(),reshape(mwArray(ONE_c),Indices(1,1,1)),true,false); // check dims but do not normalize
      }
  ...
  while(cntT<=maxM){
    ...
    *out<<cntT*delta<<"\t"<<l<<"\t"<<whichOp<<"\t"<<real(norm0)<<"\t"<<imag(norm0)<<"\t"<<real(val2_2)<<"\t"<<imag(val2_2)<<endl;
    ...
    for(int p=0;p<lenL;p++){
      mwArray auxA=mpsLop.getA(p).getA(); // xi Dl Dr
      int dP=auxA.getDimension(0);
      int sqrD=sqrt(dP);int _Dl=auxA.getDimension(1);int _Dr=auxA.getDimension(2);
      auxA.reshape(Indices(sqrD,sqrD,_Dl,_Dr));
      auxA.permute(Indices(2,1,4,3));
      auxA.reshape(Indices(dP,_Dr,_Dl));
      // without conj, now it is the evol with -t, but should I conj to compensate for later? apparently not! (to think)
      //auxMPS.setA(lenL-1-p,conjugate(auxA));
      mpsRop.replaceSite(lenL-1-p,auxA,false); // no check of dims
      // no conj, as my mpsRop is conjugated wrt true one
    }
    //mpsRop.gaugeCond('L',0); // not needed
    normFR=normFL;//+=log2(mpsRop.getNormFact());
    mpsRop.setNormFact(1.); // already true
    l++;
  }
  out->close();
  delete out;
...
void computeDistantCorrelatorsTwoBodyOpt(const MPS& mpsB,
  ...
  // // Before starting I cuold check if the symmetry holds from the beginning. compare mpsRop with the transposition of mpsLop
  // MPS auxMPS(mpsRop);
  // int lenL=mpsLop.getLength();
  // for(int p=0;p<lenL;p++){
  //   mwArray auxA=mpsLop.getA(p).getA(); // xi Dl Dr
  ...
  //   auxA.permute(Indices(2,1,4,3));
  //   auxA.reshape(Indices(dP,_Dr,_Dl));
  //   // without conj, now it is the evol with -t, but I conj to compensate for later
  //   //auxMPS.setA(lenL-1-p,conjugate(auxA));
  //   auxMPS.setA(lenL-1-p,auxA);
  //   // no conj, as my mpsRop is conjugated wrt true one
  // }
  // cout<<"Checking mpsLop and mpsRop relation at time "<<cntT*delta<<"length"<< lenL<<endl;
  // cout<<"Overlap L-R "<<contractor.contract(mpsLop,mpsRop)<<", normFR="<<normFR<<endl;
  // cout<<"Overlap with my construction "<<contractor.contract(mpsLop,auxMPS)<<", normFL="<<normFL<<endl;
  // cout<<"Overlap new-R "<<contractor.contract(mpsRop,auxMPS)<<", normNew="<<contractor.contract(auxMPS,auxMPS)
  //     <<", normR="<<contractor.contract(mpsRop,mpsRop)<<endl;
  // if(cnt>=4) exit(1);
