...
// Useful for saving and retrieving states
const string mpsTmpFileNameRoot(const string baseDir,int D,double J,double g,double h,int initSt,double delta);
...
int main(int argc,const char* argv[]){
  ...
  // This is actually ignored, now, and if the file exists or a tmp mps is found, we append. If cnt starts from 0, file is created
  bool truncH=props.getIntProperty("truncH")==1; // default is std
 ...
 // The operator I want to compute on L sites (for now, just a sigma_x in the center)
  ...
  // First, check if any intermediate state exists that I can use
  string tmpFileNameRoot=mpsTmpFileNameRoot(tmpdir,Dcut,J_,g_,h_,initSt,delta);
  string tmpMPS;
  int cnt_=M;bool found=0;
  while(cnt_>0&&!found){
    tmpMPS=mpsTmpFileName(tmpFileNameRoot,cnt_);
    if(file_exists(tmpMPS.data())){
      mpsB.importMPS(tmpMPS.data());found=true;cnt=cnt_;time=cnt*delta;app=1;
      cout<<"Found intermediate result for step "<<cnt<<endl;
    }
    // else{
    //   cout<<"No file "<<tmpMPS<<" found"<<endl;
    // }
    cnt_--;
  }
  ...
  // 3) contract expectation values
	...
	if(k<lenB)
      ...
      //clock_t finish=clock();double tappr=(finish-start)*1./CLOCKS_PER_SEC;
      //cout<<"Time to approximate cnt "<<cnt<<" "<<tappr<<endl;
      // start=clock();
      contractor.optimize(evol,aux,mpsB,Dcut);
      //finish=clock();double toptim=(finish-start)*1./CLOCKS_PER_SEC;
      //cout<<"Time to optimize cnt "<<cnt<<" "<<toptim<<" total "<<tappr+toptim<<endl;      
      //times.setElement(cnt*ONE_c,Indices(cnt,0));times.setElement((tappr)*ONE_c,Indices(cnt,1));times.setElement((toptim)*ONE_c,Indices(cnt,2));
    }
    else{ // Hastings' truncation (probably to optim!)      
      for(int nextra=0;nextra<toadd;nextra++)
	mpsB.insertSite(0,reshape(mwArray(ONE_c),Indices(1,1,1)),true,false); // check dism but do not normalize
      //mpsB=aux;
      //clock_t start=clock();
      mpsB.applyExactly(evol,false,'R'); // apply exactly the MPO, no trunc, and gauge TO the right (so Lambdat is id)
      //if(cnt>207) cout<<"After applyExactly, mpsB:"<<mpsB<<endl; 
      //clock_t finish=clock();double tapply=(finish-start)*1./CLOCKS_PER_SEC;
      //cout<<"Time to applyExactly cnt "<<cnt<<" "<<tapply<<endl;
      //start=clock();
      mpsB.gaugeCondT('L',Dcut); // then truncate with the transpose
      //if(cnt>207) cout<<"After gaugeCondT, mpsB:"<<mpsB<<endl; 
      //mpsB.gaugeCond('L',false,Dcut); // then truncate with the normal way (to check)
      //finish=clock();double tgauge=(finish-start)*1./CLOCKS_PER_SEC;
      //cout<<"Time to gaugeCondL(T) cnt "<<cnt<<" "<<tgauge<<" total "<<tapply+tgauge<<endl;
      //times.setElement(cnt*ONE_c,Indices(cnt,0));times.setElement((tapply)*ONE_c,Indices(cnt,1));times.setElement((tgauge)*ONE_c,Indices(cnt,2));
    }
    if(cutL){
      if(Lt>Lmax){
    ...
    //cout<<"cnt "<<cnt<<" mpsB norm:"<<contractor.contract(mpsB,mpsB)<<", mpsB times mpsBT:"<<contractor.contract(mpsB,bra)
    //	<<" rdm2:"<<contractor.contract(mpsB,bra,0,'R')<<endl;
    //bra.gaugeCond('R',1);
    // try gauge again? 
    MPS::gaugeCond(mpsB,bra,'L');
    mwArray rdm2=contractor.contract(mpsB,bra,0,'R'); // d2 x 1 x d2
    int D0=mpsB.getA(0).getDr();
    rdm2.reshape(Indices(D0,D0));
    rdm2.multiplyLeft(reshape(conjugate(bra.getA(0).getA()),Indices(d2,D0)));
    rdm2.multiplyRight(permute(reshape(mpsB.getA(0).getA(),Indices(d2,D0)),Indices(2,1)));
    ...
    // rdm2.reshape(Indices(d2*d2,1));
    // cout<<"Found rdm for step "<<cnt<<", namely "<<rdm2.getDimensions()<<endl;
    saveObservables(rdm2,cnt,delta,outfname,J_,g_,h_);

    if(savingTmp&&(cnt%savingfreq==0)){ // save tmp MPS
      string newtmpMPS=mpsTmpFileName(tmpFileNameRoot,cnt);
      mpsB.exportMPS(newtmpMPS.data());
      cout<<"Saved intermediate result to file "<<newtmpMPS<<endl;
      if(file_exists(tmpMPS.data())){
	remove(tmpMPS.data());
      }
      tmpMPS=newtmpMPS;
...
const string mpsTmpFileNameRoot(const string baseDir,int D,double J,double g,double h,int initSt,double delta){
  stringstream s;
  s<<baseDir<<"/tmpMPS_D"<<D<<"_J"<<J<<"_g"<<g<<"_h"<<h
   <<"_quenchFrominit"<<initSt<<"_delta"<<delta;
  s<<"_";
  return s.str();
}
