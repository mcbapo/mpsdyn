...
double computeRDMdistanceToId(const mwArray& rdm,int Lc,double& entro);
...
int main(int argc,const char* argv[]){
  ...
  string Sredfname=props.getProperty("outputSred"); // for the entropy of the block
  ...
  if(Sredfname.size()>0){
	...
	exit(1);
      ...
      if(ising){
	*outSred<<"% N\t J\t g\t h\t D\t cnt\t Lc(1)\t Sred(Lc)..."<<endl;
      }
      else{
	if(!isXY)
	  *outSred<<"% N\t Jx\t Jy\t Jz\t h\t D\t cnt\t Lc(1)\t Sred(Lc)..."<<endl;
	else
	  *outSred<<"% N\t Jx\t Jy\t hx\t hy\t hz\t D\t cnt\t lambda[0]^2\t lamdba[1]^2..."<<endl;
      }
      outSred->close();
      delete outSred;
    }
  ...
  for(int k=cnt;k<=M;k++){
	...
	*outS<<schmidt[vS]<<"\t";
	...
	if(!avrg){
	  ...
	  entropyRDM=vector<double>(sizeRDM.size(),0.);
	  ...
	  while(posL<L-Lb-1){ // at least there is a 2-site RDM to compute
		...
		double entro_lc=0;
		double dist_lc=computeRDMdistanceToId(oper,lc,entro_lc);
		//double entro_lc=computeRDMentropy(oper,lc);
		trDistRDM[lc-1]+=dist_lc;
		entropyRDM[lc-1]+=entro_lc;
	  ...
	  for(int lc=1;lc<=Lcmax;lc++){
	    trDistRDM[lc-1]=trDistRDM[lc-1]/avrgCnt[lc-1];
	    entropyRDM[lc-1]=entropyRDM[lc-1]/avrgCnt[lc-1];
	  }
	}
      }    
	...
	*outSred<<L<<"\t"<<J_<<"\t"<<g_<<"\t"<<h_<<"\t"<<D<<"\t";
	...
	if(!isXY)
      ...
      *outSred<<k<<"\t";
      if(checkRDM){
	for(int ks=0;ks<sizeRDM.size();ks++){
	  *outSred<<sizeRDM[ks]<<"\t"<<entropyRDM[ks]<<"\t";
	}
      }
      *outSred<<endl;
	...
	*outEd<<endl;
    ...
    if((saveFr>0&&k%saveFr==0&&k>0)||(M-k<=1)){
