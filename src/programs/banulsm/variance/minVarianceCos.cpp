...
/** To find the GS of H^2, instead of running findGroundState,
    ...
    or the HeisenbergHamiltonian (in which case different parameters are expected) 
...
const MPO& getHamilMPO(Hamiltonian& hamil,bool ising);
...
int main(int argc,const char* argv[]){
  ...
  bool ising=(props.getIntProperty("ising")!=0); // only if -ising=0 is given it will be Heisenberg  
  ...
  string Sfname=props.getProperty("outputS"); // for Schmidt values (to have an idea of truncation)
  ...
  Hamiltonian* ham;
  if(ising){
    ham=new IsingHamiltonian(L,d,J,g,h);
  }
  else{
    ham=new HeisenbergHamiltonian(L,Jx,Jy,Jz,h);
  }
  cout<<"Created the Hamiltonian"<<endl;
  const MPO& hamil=ham->getHMPO();
  cout<<"Constructed the hamil MPO"<<endl;

  cout<<"Initial state has <H>="<<contractor.contract(gs,hamil,gs)<<", <H^2>="<<contractor.contract2(hamil,gs)
      <<", S(L/2)="<<contractor.getEntropy(gs)
      <<endl;
  ...
  if(!app){
    ...
    if(ising){
      ...
      *out<<"% N\t Jx\t Jy\t Jz\t h\t D\t dt\t deltaX\t cnt\t time \t<H2>\t <H>";
    }
    if(checkRDM){
  ...
  if(!file_exists(Sfname.data())){
    ...
    if(ising){
      *outS<<"% N\t J\t g\t h\t D\t dt\t deltaX\t cnt\t time \t lambda[0]^2\t lamdba[1]^2..."<<endl;
    }
    else{
      *outS<<"% N\t Jx\t Jy\t Jz\t h\t D\t dt\t deltaX\t cnt\t time lambda[0]^2\t lamdba[1]^2..."<<endl;
    }
    *outS<<endl;
    outS->close();
    delete outS;
  ...
  // In the case of Heisenberg, many more terms needed:He/2, He and Ho for each positive and negative
  MPO expPe2(L),expPe(L),expPo(L);
  MPO expMe2(L),expMe(L),expMo(L);
  if(ising){
    IsingHamiltonian* hamI=static_cast<IsingHamiltonian*>(ham);
    ...
    HeisenbergHamiltonian* hamH=static_cast<HeisenbergHamiltonian*>(ham);
    hamH->getExponentialMPOeven(expPe2,dt*.5*I_c);
    hamH->getExponentialMPOeven(expPe,dt*I_c);
    hamH->getExponentialMPOodd(expPo,dt*I_c);
    if(!realInit){
      hamH->getExponentialMPOeven(expMe2,-dt*.5*I_c);
      hamH->getExponentialMPOeven(expMe,-dt*I_c);
      hamH->getExponentialMPOodd(expMo,-dt*I_c);
    }
  }
  ...
  while(stepsLeft>0){
    ...
    if(ising){ // Ising Hamiltonian: single operator for one step
    ...
    else{ // Heisenberg Hamiltonian: each step needs several MPOs
      start=clock();int stepsInTime=nrToApply;
      for(int k=0;k<nrToApply;k++){
	// If mS>1, need to apply several operators of each sign before summing
	MPS aux1(gs); // for positive exp
	MPS aux2(gs); // for neg exp
	for(int ik=0;ik<mS;ik++){
	  MPS _aux1(aux1);
	  if(ik==0)	    
	    contractor.optimize(expPe2,_aux1,aux1,D);
	  contractor.optimize(expPo,aux1,_aux1,D);
	  if(ik<mS-1) contractor.optimize(expPe,_aux1,aux1,D);
	  else  contractor.optimize(expPe2,_aux1,aux1,D);
	  if(!realInit){
	    MPS _aux2(aux2);
	    if(ik==0)	    
	      contractor.optimize(expMe2,_aux2,aux2,D);
	    contractor.optimize(expMo,aux2,_aux2,D);
	    if(ik<mS-1) contractor.optimize(expMe,_aux2,aux2,D);
	    else contractor.optimize(expMe2,_aux2,aux2,D);
	  }
	}
	aux1.gaugeCond('R',1);
	if(!realInit){
    ...
    Shalf=contractor.getEntropy(gs);
    ...
    *out<<endl;
    ...
    double entroSch=getSchmidtVals(schmidt,gs,L/2-1,'R');
    // debugging
...
void initialState(MPS& mps,int initSt,int D){
  ...
  if(initSt==+1||abs(initSt)==4) mps.setProductState(p_xplus);
	...
	if(initSt==-2) mps.setProductState(p_yminus);
	  ...
	  if(initSt==+3||abs(initSt)==6||abs(initSt)>=10) mps.setProductState(p_zero);
	  else{
  ...
  if(abs(initSt)==10){
    ...
    // int firstPos=initSt<0?0:2; //++-- or --++
    // for(int k=firstPos;k<L;k+=4){
    // 	mps.applyLocalOperator(k,oper);
    // 	if(k+1<L) mps.applyLocalOperator(k+1,oper);
    // }    
  }
  mps.increaseBondDimension(D);
  // if(abs(initSt)==11){
  //   // A test: rotate every spin a little bit
  //   complex_t datax[]={ZERO_c,ONE_c,ONE_c,ZERO_c};
  //   mwArray oper=mwArray(Indices(d,d),datax);
  //   for(int k=1;k<L;k++){
  //     mwArray operK;
  //     wrapper::expm(oper,operK,-I_c*M_PIl*k*1./L);
  //     mps.applyLocalOperator(k,operK);
  //   }
  // }
}
