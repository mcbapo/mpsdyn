...
/** Try to approximate the diagonal ensemble by projecting the initial
    ...
    approximation for the projector (but those involve higher powers
    every step! e.g. 3^k instead of 2^k, so for this version I don't
    think there is any difference).

    Here we try a weaker version acting only on the state (although
    the nr of applications of the operator then needs to be
    exponential with the nr of steps, to get the same exact result)
    The algorithm just applied P_0=(Id-scale*Hc*Hc^dagger) every step.
    Notice that quadratic convergence results refer to the k-th order
    approximation of the null projector, which is (P_0)^(2^k)
...
int main(int argc,const char* argv[]){
  ...
  *out<<0<<"\t"<<D<<"\t"<<real(tr0)<<"\t"<<imag(tr0)<<"\t"
  ...
  idMPSdoub.gaugeCond('R',0);idMPSdoub.gaugeCond('L',0);
  double normP0=log2(idMPSdoub.getNormFact()); // actually, this I know exactly: 2*L?
  idMPSdoub.setNormFact(1.);
  ...
  double normP=normP0;
  {
    ...
    double normHc=log2(aux.getNormFact()); // which is tr(Op^2)
    aux.setNormFact(1.);
    MPO comm2(L);
    extendMPO(commMPO,comm2,d*d);
    //    cout<<"comm2="<<comm2<<endl;
    MPS aux2(aux);
    int Dhc=2+2*(Dh-2); // bond dim of commutator
    contractor.optimize(comm2,aux2,aux,min(D,Dhc*Dhc));
    aux.gaugeCond('R',0);aux.gaugeCond('L',0);
    normHc+=log2(aux.getNormFact());
    aux.setNormFact(1.);
    ...
    coeffs.push_back(-scale*pow(2,normHc-normP0)*ONE_c); // coeff of the Hc Hc
    contractor.optimizeSum(vecs,coeffs,projNull,min(D,Dhc*Dhc+1)); // Do I want this to be exact? Dh*Dh+1
  ...
  while(cnt<=M){
	...
	MPO::join(4,mpos,Pcomm2P);
      ...
      // Instead, the commutator for the previous step
      valC2=contractor.contract2(commMPO,rho0);
    ...
    *out<<cnt<<"\t"<<D<<"\t"<<real(trM)<<"\t"<<imag(trM)<<"\t"
	...
	<<" <Sz>="<<real(valzM)/real(trM);
      if(0)
	cout<<" |[Hc,rho]|^2="<<real(valC2)*pow(2,2*normP)*pow(2,2*normMPS);
      else
	cout<<" |[Hc,rho]|^2="<<real(valC2)*pow(2,2*normMPS);      
      cout<<endl;
    // one step further
    MPS aux(rho0);
    contractor.optimize(projMPO,aux,rho0,D);
    rho0.gaugeCond('R',0);rho0.gaugeCond('L',0);
    normMPS+=log2(rho0.getNormFact())+normP;
    rho0.setNormFact(1.);
    cnt++;
  }
}
