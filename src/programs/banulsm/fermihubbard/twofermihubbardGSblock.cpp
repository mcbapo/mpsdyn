...
/** twofermihubbardGSblock runs the findGroundState routine with the MPO for the
    Hamiltonian \ref <TwoOrbitalFermiHubbardHamiltonianBlock>, and computes GS and
    maybe other things... Here each site in the chain is one real space site, containing 4 fermionic modes!
...
int main(int argc,const char* argv[]){
  ...
  if(noise<0) noise=0.; // whether to perturb existing tmp or smaller D solutions
  ...
  MPS gs(L,D,pow(d,4));gs.setRandomState();gs.gaugeCond('R',1);
  ...
  if(file_exists(mpsfile)){
	...
	exit(1);
      ...
      if(noise>0)
	gs.increaseBondDimensionWithNoise(D,noise);
      else
	gs.increaseBondDimension(D);
    }
    else{
  ...
  cout<<"Initial value of the energy: "<<E0<<endl;
  ...
  if(warmup){
	...
	for(int pos=0;pos<(L-1)/2+1;pos++){
	  ...
	  contractor.sweepPart(L-1-pos,L-1-pos-1,hamil,D,gs,0.); // one left
	}
	...
	for(int pos=0;pos<(L-1)/2+1;pos++){
	  int leftB=L/2-1-pos;int rightB=4*L/2+pos;
  ...
  if(penNtot+penNg+penNe+penSz>0){
    ...
    if(penSz>0){
      ...
      goodsector=goodsector&&checkQuantumNumber(gs,fermSz,Sz,.5*2.,1E-3);
    }
    if(penNtot>0){
  ...
  cout<<"If I compute it directly:"<<contractor.contract(gs,hamil,gs)<<endl;
  ...
  *out<<endl;
  ...
  if(relaunch){// prepare the next job
    ...
    if(penSz>0&&abs(real(2*valSz)-Sz)>1E-2){ // got the wrong Sz
      cout<<"Result with wrong 2Sz "<<2*valSz<<endl;
