...
int main(int argc,const char* argv[]){
  ...
  if(!restart){
	...
	if(relaunch){
	  int nextD=D+incrD;
	  if(nextD<=maxD)
	    prepareNewJob(jobsdir,basempsfile,nextD,penNtot,penNg,penNe,penSz,argv[0],argv[1],props);
	}
	exit(0);
      }
      else{
  ...
  else{ // restart only runs if the same MPS file is found refine runs even if
    mpsfile=mpsFileName(mpsdir,basempsfile,Drestart);
    if(file_exists(mpsfile)){
      gs.importMPS(mpsfile.data());
    }
    else{
      cout<<"Running with option restart => cannot run without file "<<mpsfile<<endl;
      exit(1);
    }
  }
  gs.gaugeCond('R',1);
  ...
  // if tryVs is given, I will try to recover GS from neighbouring Vs and check their energies
  if(tryVs){
    double Vl=V-incrVtry;double Vexl=Vex+.5*incrVtry;
    string basempsfileL=getMPSfilename(L,t,U,Vl,Vexl,mu_g,mu_e,1); // basename of MPS files
    if(penNtot+penNg+penNe+penSz>0)
      basempsfileL=getMPSfilenameConstrains(L,penNg,Ng,penNe,Ne,penSz,Sz,penNtot,Ntot,t,U,Vl,Vexl,mu_g,mu_e,1); // basename of MPS files
    int Dtry=D;bool found=0;
    while(Dtry>0&&!found){
      string mpsfileL=mpsFileName(mpsdir,basempsfileL,Dtry);
      if(file_exists(mpsfileL)){
	MPS gsL;
	gsL.importMPS(mpsfileL.data());
	double El=real(contractor.contract(gsL,hamil,gsL));
	if(El<E0){
	  cout<<"Found MPS for V="<<Vl<<" with lower energy=>using it"<<endl;
	  gs=gsL;
	  E0=El;found=true;
	}
	else{cout<<"File "<<mpsfileL<<"does not work"<<endl;}
      }
      //else  cout<<"No MPS found for V-1 at "<<mpsfileL<<endl;
      Dtry--;
    }
    double Vr=V+incrVtry;double Vexr=Vex-.5*incrVtry;
    string basempsfileR=getMPSfilename(L,t,U,Vr,Vexr,mu_g,mu_e,1); // basename of MPS files
    if(penNtot+penNg+penNe+penSz>0)
      basempsfileR=getMPSfilenameConstrains(L,penNg,Ng,penNe,Ne,penSz,Sz,penNtot,Ntot,t,U,Vr,Vexr,mu_g,mu_e,1); // basename of MPS files
    Dtry=D;found=0;
    while(Dtry>0&&!found){
      string mpsfileR=mpsFileName(mpsdir,basempsfileR,Dtry);
      if(file_exists(mpsfileR)){
	MPS gsR;
	gsR.importMPS(mpsfileR.data());
	double Er=real(contractor.contract(gsR,hamil,gsR));
	if(Er<E0){
	  cout<<"Found MPS for V="<<Vr<<" with lower energy=>using it"<<endl;
	  gs=gsR;found=true;
	}
	else{cout<<"File "<<mpsfileR<<"does not work"<<endl;}
      }
      //else cout<<"No MPS found for V+1 at "<<mpsfileR<<endl;      
      Dtry--;
    }
  }
