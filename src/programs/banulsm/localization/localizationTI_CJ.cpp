...
/** localizationTI_CJ simulates the Choi-Jamiolkowski state
    corresponding to the evolution operator for the special
    Hamiltonian \ref <HeisenbergAncillaryNoiseHamiltonian>, in which
    the effect of the random magnetic field is achieved by a second
    spin chain, interacting with the first via an Ising type
    term. 

    This is achieved by simulating the time evolution of a spin chain
    where for every spin in the ladder there is an ancilla, with which
    the spin is maximally entangled at the beginning.  Formally this
    is the same as starting an identity MPO and then applying
    evolution only on one side.
    
    \NOTICE: Alternatively, I could apply one half of the evolution on
    each side of this MPO if it were better.

    At the end I want to compute the entropy of half chain, maybe also 
    the truncation error, the entropy of some blocks in the middle of 
    the chain (costly! D^4 matrix, D^6 diagonalization!)... maybe I 
    leave this for later, once I've saved the states. I could actually 
    fire the job that computes this for a particular state once I have 
    written the MPS!! (TODO!!!!)
...
int main(int argc,const char* argv[]){
  ...
  *out<<setprecision(15);
  ...
  // In this case, I am only using single side operators, but I keep 
  // the code for the double side, in case I decide to split the evolution
  MPO expHSe(2*L),expHSo(2*L),expHSo2(2*L),
    expHTe(2*L),expHTo(2*L),expHTo2(2*L),
    expHB(2*L),expHB2(2*L);
  // Only on one side, then I need to extend them
  {
    MPO aux(2*L);
    hamH.getExponentialMPOeven(aux,-delta*I_c,true);
    extendMPO(aux,expHSe,d);
    hamH.getExponentialMPOodd(aux,-delta*I_c,true);
    extendMPO(aux,expHSo,d);
    hamH.getExponentialMPOodd(aux,-delta*.5*I_c,true);
    extendMPO(aux,expHSo2,d);
    hamH.getExponentialMPOeven(aux,-delta*I_c,false);
    extendMPO(aux,expHTe,d);
    hamH.getExponentialMPOodd(aux,-delta*I_c,false);
    extendMPO(aux,expHTo,d);
    hamH.getExponentialMPOodd(aux,-delta*.5*I_c,false);
    extendMPO(aux,expHTo2,d);
    hamH.getExponentialMPOmixed(aux,-delta*I_c);
    extendMPO(aux,expHB,d);
    hamH.getExponentialMPOmixed(aux,-delta*.5*I_c);
    extendMPO(aux,expHB2,d);
  }
  // The double ones
...
//   MPSfromMPO(hamil,hmps);
  ...
  // Now do the evolution step by step, and compute the entropy of half chain and the energy
  MPO hamil(2*L);
  const MPO& _hamil=hamH.getHMPO();
  extendMPO(_hamil,hamil,d);
  int cnt=M0;
  double time=cnt*delta;
  complex_t Eev; double Shalf;
  cout<<"Time evolution step nr "<<cnt<<", time="<<time<<endl;
  Eev=contractor.contract(rho0,hamil,rho0);
  Shalf=contractor.getEntropy(rho0);
  *out<<cnt<<"\t"<<delta<<"\t"<<time<<"\t"<<D<<"\t"<<real(Eev)<<"\t"<<imag(Eev)
      <<"\t"<<Shalf;
  ...
  *out<<endl;
  ...
  while(cnt<Mtot){
    ...
    cout<<"Time evolution step nr "<<cnt<<", time="<<time<<", E value "<<Eev<<", entropy "<<Shalf<<endl;
    out=new ofstream(outfname,ios::app);
    if(!out->is_open()){
      cout<<"Error: impossible to open file "<<outfname<<
	" for output"<<endl;
