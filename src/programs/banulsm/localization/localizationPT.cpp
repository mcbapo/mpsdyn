...
/** localizationPT simulates the time evolution of an MPO under a
    ...
    \param <initState> (int) which initial tho to consider. Options are:
                        0 - \f$M_1=\sum cos(n \pi/L) \sigma_z^{[n]}\f$
			1 - \f$M_1=\sigma_z^{[L/2]}\f$
			2 - \f$M_1=\sum exp(i 2 n \pi/L) \sigma_z^{[n]}\f$
...
int main(int argc,const char* argv[]){
  ...
  int seed=0;
  if(argc>12) seed=atoi(argv[++cntr]);
  ...
  *out<<"% MPS in "<<mpsfile<<endl;
  ...
  for(int k=0;k<L;k++){
    ...
    //    h[k]=h[k]>=0?h_:-h_; // for discrete values!!
  }
  cout<<"h="<<h<<endl;
  ...
  *out<<"% M\t delta\t t\t D\t <M1>\t tr(rho)\t tr(rho H)"<<endl;
  ...
  cout<<"Created the Hamiltonian"<<endl;
  {
    ...
    MPS testGS(L,D,d);double lambda=0;
    contractor.findGroundState(hamH.getHMPO(),D,&lambda,testGS);
    hamH.getHMPO().exportForMatlab("hamilHeis.m");  
  ...
  // I construct the simple ones first, and then from them the DoubleOperators
  {
    ...
    for(int k=0;k<L;k++){
      expHe2.setOp(k,new DoubleOperator(_expHe2.getOp(k).getFullData(),permute(_expHe2A.getOp(k).getFullData(),Indices(3,2,1,4))),true);
      expHe.setOp(k,new DoubleOperator(_expHe.getOp(k).getFullData(),permute(_expHeA.getOp(k).getFullData(),Indices(3,2,1,4))),true);
      expHo.setOp(k,new DoubleOperator(_expHo.getOp(k).getFullData(),permute(_expHoA.getOp(k).getFullData(),Indices(3,2,1,4))),true);
  ...
  MPS M1(L,2,d*d);
  switch(initSt){
  case 0: // Slowest mode (cos nPi/L)
    constructM1(L,d,rho0);
    M1=rho0;
  break;
  case 1: // localized in site L/2
    constructLocal(L,L%2==0?L/2-1:(L-1)/2,d,rho0);
    constructM1(L,d,M1);
    break;
  case 2: // Complex exponential
    constructM1exp(L,d,rho0);
    M1=rho0;
    break;
  default:
    cout<<"ERROR: Unknown initial state option "<<initSt<<" (valid only 0,1,2)"<<endl;
    exit(initSt+1);
  }
  //  
