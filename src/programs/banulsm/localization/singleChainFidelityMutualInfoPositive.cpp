...
/** Based on qubitSingleChain and mutualInfoSingleChain, this program 
    ...
    \f$\rho_{1/2}=\left ( \frac{Id}{\sqrt{d}}\right )^{\otimes L-L_0}
    \otimes |\phi_0\rangle \langle \phi_0| \f$ (so, basically
    \f$\rho\f$ with different normalization, and the evolved state can
    be written as \f$rho(t)=\rho_{1/2}(t)^{\dagger} \rho_{1/2}(t)\f$,
    ...
    (MPO-origami) tricks. Some optimization would be needed for the 
    expandOper step.
*/
...
/** Vectorize an MPO to make an MPS out of it. If up==true (default)
    ...
    which was going up. If conj==true, tensors are complex conjugated*/
...
void computeTraceNormsPurification(const MPS& rho0p,const MPS& rho0m,const char* outfname,int L,int L0,bool isXY,double J_,double h_,int initSt,
  ...
  *out<<setprecision(15);
  ...
  // First I impose gauge condition on the left and right parts (outside L0)
  for(int k=0;k<=max(L/2-L0/2-1,-1);k++){
    rhop.gaugeCond(k,'R',true);
    rhom.gaugeCond(k,'R',true);
  }
  for(int k=L-1;k>=min(L/2+L0/2,L);k--){
...
void complementaryMPO(const MPS& rho,const vector<int>&toTrace,MPO& mpo,bool dagger=false){
  ...
  mpo.initLength(toKeep.size()+2);
  int Dl=rho.getA(toKeep.front()).getDl();
  mwArray IdDl=identityMatrix(Dl);
  if(!dagger)
    IdDl.reshape(Indices(Dl,1,1,Dl));
  else
    IdDl.reshape(Indices(1,1,Dl,Dl));
  ...
  for(int k=0;k<toKeep.size();k++){
    ...
    mpo.setOp(k+1,new Operator(aux),true);
  }
  mpo.setOp(0,new Operator(IdDl),true);
  mpo.setOp(toKeep.size()+1,new Operator(IdDr),true);
}
...
double traceNormDiffPurification(const MPS& rhop,const MPS& rhom,const vector<int>& toTrace,bool gauge){
  ...
  //  cout<<"complementaryMPO of rhoP found: "<<rhoMPO<<endl; 
  //MPO rhoTot(L);
  //complementaryMPO(rhop,toTrace,rhoMPOd,true); // rho dagger
  //cout<<"complementaryMPO of rhoPd found: "<<rhoMPOd<<endl; 
  //const MPO* aux[2];
  //aux[0]=&rhoMPOd;aux[1]=&rhoMPO;
  //MPO::join(2,aux,rhoTot);
  //expandOper(rhoTot,OpP);
  // complementaryMPO(rhom,toTrace,rhoMPO);
  //cout<<"complementaryMPO of rhoM found: "<<rhoMPO<<endl; 
  //  complementaryMPO(rhom,toTrace,rhoMPOd,true);
  //cout<<"complementaryMPO of rhoMd found: "<<rhoMPOd<<endl; 
  //MPO::join(2,aux,rhoTot);
  //expandOper(rhoTot,OpM);
...
//       <<trR2_c<<", normalization="<<normRho<<", RESULT="<<-log2(trR2/normRho)<<endl;
  ...
  // For trace(rho^2) I construct an MPO that swaps up and down
  ...
  *out<<setprecision(15);
  ...
  for(int posC=1;posC<=L/2;posC++){
    if(!header){
      ...
      compute2RenyiEntropiesPurificationAB(rho,0,posC-1,&Sa,&Sbc);
      compute2RenyiEntropiesPurificationAB(rho,posC,L-posC-1,&Sb,&Sac);
      *out<<Sa<<"\t"<<Sbc<<"\t"<<Sb<<"\t"<<Sac<<"\t";
    }
    else{
