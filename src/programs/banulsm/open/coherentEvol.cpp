...
/** 
    Program coherentEvol: Try to compute the steady state of a certain
    Liouvillian
    as in \class <CoherentDissipationLiouvillian>,
    with g, gamma given as input parameters.
    In this case, I will compute it by applying the 
    imaginary time evolution \f$e^{-\tau L+L}\f$
    a few times between calls to findGroundState

    To compile: make coherentEvo
...
int main(int argc,const char* argv[]){
  ...
  int nSteps=props.getIntProperty("nsteps");
  ...
  // one line)
  sprintf(fileNameC,"coherentEvoSingle_%d_%1.2f_%D.txt",L,gamma,D);
  ...
  sprintf(fileNameC,"coherentEvoDouble_%d_%1.2f_%D.txt",L,gamma,D);
  ...
  // Now apply the exponential
  for(int k=0;k<nSteps;k++){
    MPS aux(myGS);
    contractor.approximateExponential(LdaggerL,aux,myGS,
				      tau,D,true,offset,scal);
    cout<<"After step "<<k<<" exp value is"
  ...
  cout<<"Finished second GS search with lambda="<<lambda<<endl;
