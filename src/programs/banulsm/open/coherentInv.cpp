...
/** 
    Program coherentInv: Try to compute the steady state of a certain
    Liouvillian
    as in \class <CoherentDissipationLiouvillian>,
    with g, gamma given as input parameters.
    In this case, I will compute it by applying the power method to (L+ L)^(-1)

    To compile: make coherentInv
    ...
    The names of the properties that can be assigned are:
    ...
    + noise [OPTIONAL] (double) magnitude of the (global) noise to be
                    added to the expanded tensors if we start from an
		    oldmps file with smaller D
...
int main(int argc,const char* argv[]){
  ...
  // one line)
  sprintf(fileNameC,"coherentInvSingle_%d_%1.2f_%D.txt",L,gamma,D);
  ...
  sprintf(fileNameC,"coherentInvDouble_%d_%1.2f_%D.txt",L,gamma,D);
  ...
  while(!done){
    MPS aux(myGS);
    //    contractor.optimizeInv(LdaggerL,aux,myGS,D);
    contractor.optimizeInv(Lop,aux,myGS,D);
    double newlambda=myGS.getNormFact();
    round++;
    if((abs(lambda-newlambda)<tol*abs(lambda))||(round>MAXROUND)) done=true;
    lambda=newlambda;
    myGS.setNormFact(1.);
    cout<<"Iteration "<<round<<", eigenvalue (of L) "<<newlambda<<" and expectation value "
	<<contractor.contract(myGS,LdaggerL,myGS)<<endl;
  }
  clock_t end=clock();

  cout<<"Finished SS search with lambda="<<contractor.contract(myGS,LdaggerL,myGS)<<endl;
