...
/** 
    ...
    + mpsfile (string) root of the filename where the resulting 
                 MPS will be stored. They will hold an automatic suffix.
    + nrlev (int) number of levels to compute (four)
    + oldmpsdir [OPTIONAL] (string) name of a director where reference
                 MPS files are stored that can be used as starting point.
...
/** Find if there exists a valid initial MPS file for the relevant
    parameters and level */
bool lookForRefMPSFile(int lev,const string& mpsdir,string& filename,const Properties& props);

/** Initialize the MPS form the chosen file, with stretching, or
    noise filling, if needed */
void initializeMPSfromfile(MPS& myGS,const string& oldMPSfile,bool usingOldMPS,const Properties& props);
...
int main(int argc,const char* argv[]){
  ...
  string oldMPSfile;
  if(usingOldMPS)
    usingOldMPS=lookForRefMPSFile(0,oldMPSdir,oldMPSfile,props);
  initializeMPSfromfile(myGS,oldMPSfile,usingOldMPS,props);
  ...
  sprintf(fileNameC,"MPS_%d_%1.2f_%1.2f_%d_0.mat",L,g,gamma,D);
  const string newMPSfile=newMPSdir+"/"+fileNameC;
  myGS.exportMPS(newMPSfile.data());
  cout<<"Saved first level found ("<<lambda<<") to file "<<newMPSfile<<endl;

  // compute other excitations
  vector<MPS*> levels;
  ...
  if(nrLevs>1){
    ...
    while(compL<nrLevs){
      ...
      usingOldMPS=lookForRefMPSFile(compL,oldMPSdir,oldMPSfile,props);
      initializeMPSfromfile(*exc,oldMPSfile,usingOldMPS,props);
      double lambda1=0.;
      double excOffset=-1;
      contractor.findNextExcitedState(LdaggerL,D,levels,&lambda1,*exc,excOffset);
      sprintf(fileNameC,"MPS_%d_%1.2f_%1.2f_%d_%d.mat",L,g,gamma,D,compL);
      const string levelMPSfile=newMPSdir+"/"+fileNameC;
      exc->exportMPS(levelMPSfile.data());
      cout<<"Saved next level found ("<<lambda1<<") to file "<<levelMPSfile<<endl;
  ...
  // could start computing correaltions afterwards) 
...
bool lookForRefMPSFile(int lev,const string& mpsdir,string& filename,const Properties& props){
  cout<<"Looking for file for level "<<lev<<endl;
  int L=props.getIntProperty("L");
  int initL=props.getIntProperty("initL");
  if(initL<0) initL=L;
  int D=props.getIntProperty("D");
  ...
  double g=props.getDoubleProperty("g");
  double gamma=props.getDoubleProperty("gamma");
  bool usingOldMPS=false; // only if found, will use a file
  ...
  while(it-->allDs.begin()&&!found){
    ...
    if(*it<=D){
      char oldFile[MAXLEN];
      sprintf(oldFile,"MPS_%d_%1.2f_%1.2f_%d_%d.mat",initL,g,gamma,*it,lev);
      filename=mpsdir+"/"+oldFile;
      //cout<<"Should be file "<<filename<<endl;
      ifstream test(filename.data());
      if(test){
	cout<<"Found reference file! in "<<filename<<endl;
