...
/** 
    Program coherentPropsLRprojS1: Compute the steady state of a certain
    Liouvillian
    as in \class <CoherentDissipationLiouvillianLR>,
    with g, gamma given as input parameters.
    Try to add a projector to the L (in this case, the projector onto S=1 
    for each pair of adjacent spins)
    (TODO) Use a penalty term (mu) to select the S=N/2 subspace.

    To compile: make coherentLRS1
...
/** Same thing but on a projected part of rho, found by applying a 
    (double) MPO (such as S1xS1). */
int computeSingleBodyProjEV(const MPO& projMPO,const MPS& rho,const mwArray& op1,ostream& os);
...
void computeTwoBodyProjEV(const MPO& projMPO,const MPS& rho,const mwArray& op1,
		      const mwArray& op2,int l,ostream& os,
		      int maxLen=0);
...
/** Given a regular MPO as it would act on a vector, construct the
    double MPO that will act on both sides of the linearized rho
    (typically a projector). If adjoint==true (default false), the MPO
    acting on the right is the Hermitian conjugate. */
void doubleMPO(const MPO& mpo,MPO & doubleMPO,bool adjoint=false);
...
int main(int argc,const char* argv[]){
  ...
  // now construct the double mpo to apply on both sides of rho
  MPO S1doub(L);
  doubleMPO(Stot1,S1doub,true);

  // Auxiliary, conjugate by hand the S1 projector
  MPO Stot1Dagger(L);
  for(int pos=0;pos<L;pos++){
    mwArray aux=Stot1.getOpData(pos);
    aux.permute(Indices(3,2,1,4),true);
    Stot1Dagger.setOp(pos,new Operator(aux),true);
  }
  // And construct the adjoint double MPO too
  MPO S1doubDagger(L);
  doubleMPO(Stot1Dagger,S1doubDagger,true);

  // for initial state
  MPS S1mps(L,4,d*d);
  MPSfromMPO(Stot1,S1mps);

  // And now construct a Joined MPO for the L+ L
  // Plus, in this case, the projector on every pair S=1

  const MPO* ptrs[4]={&S1doub,&Ldagger,&Lop,&S1doub};
  MPO LdaggerL(L);
  MPO::join(4,ptrs,LdaggerL);
  ...
  if(usingOldMPS){
	...
	else{ // mode is 6, indicating start with S2 oper or 7 with S1 projector
	  if(mode==6){
	    myGS=S2mpsTrueU;
	  }
	  if(mode==7){
  ...
  cout<<"Starting from an energy value "<<lambda<<" and initial <S2>="
      <<iniS<<", and initial trace="<<traceRho<<endl;
  if(wur>0){
    cout<<"Applying "<<wur<<" symmetric warm up rounds"<<endl;
    for(int pp=0;pp<wur;pp++){
      for(int pos=0;pos<(L-1)/2+1;pos++){
	contractor.sweepPartWithProjPenalty(pos,pos+1,LdaggerL,D,mu,
					    S2mps,myGS,offset); // one right
	contractor.sweepPartWithProjPenalty(L-1-pos,L-1-pos-1,LdaggerL,D,mu,
			     S2mps,myGS,offset); // one left
      }
      lambda=real(contractor.contract(myGS,LdaggerL,myGS));
  ...
  clock_t start=clock();
  vector<double> multipliers(1,-1.*mu);
  ...
  //  lambda=real(contractor.contract(myGS,LdaggerL,myGS));
  traceRho=getTraceRho(myGS);
  _s2_=contractor.contract(myGS,S2mpsTrue);
  cout<<"Finished GS search with lambda="<<lambda<<", <rho|S1 L+L S1|rho>"<<contractor.contract(myGS,LdaggerL,myGS)<<endl;
  cout<<"And S^2="<<real(_s2_/traceRho)<<endl;
  cout<<"And < S^2 S1 rho S1>/<S1 rho S1>="<<real(contractor.contract(myGS,S1doub,S2mpsTrue)/getEVRho(myGS,S1doub))<<endl;
  cout<<"And trace(S1 rho S1)="<<getEVRho(myGS,S1doub)<<endl;
  ...
  complex_t traceSRho=getEVRho(myGS,S1doub);
  _s2_=contractor.contract(myGS,S1doub,S2mpsTrue);
  *out1<<"% and <S2 S1 rho S1>="<<real(_s2_/traceSRho)<<endl;
  *out1<<"% and trace(rho S1)="<<traceSRho<<endl;

  int errors=0;
  // Compute now expectation values of sigmaX,Y,Z
  *out1<<0<<"\t"<<1<<"\t";
  errors=computeSingleBodyProjEV(S1doub,myGS,sigX,*out1);
  *out1<<endl;
  *out1<<0<<"\t"<<2<<"\t";
  errors=computeSingleBodyProjEV(S1doub,myGS,sigY,*out1);
  *out1<<endl;
  *out1<<0<<"\t"<<3<<"\t";
  errors=computeSingleBodyProjEV(S1doub,myGS,sigZ,*out1);
  *out1<<endl;
  out1->close();
  delete out1;
  ...
  *out2<<"% Every line starts with an integer: 0 for the lowest eigenvalue "
  ...
  for(int l=1;l<=L/2;l++){
    *out2<<0<<"\t"<<1<<"\t"<<l<<"\t";
    computeTwoBodyProjEV(S1doub,myGS,sigX,sigX,l,*out2,maxLen);
    *out2<<endl;
    *out2<<0<<"\t"<<2<<"\t"<<l<<"\t";
    computeTwoBodyProjEV(S1doub,myGS,sigY,sigY,l,*out2,maxLen);
    *out2<<endl;
    *out2<<0<<"\t"<<3<<"\t"<<l<<"\t";
    computeTwoBodyProjEV(S1doub,myGS,sigZ,sigZ,l,*out2,maxLen);
    *out2<<endl;
  }
  out2->close();
  delete(out2);
...
int computeSingleBodyProjEV(const MPO& projMPO,const MPS& rho,const mwArray& op1,ostream& os){
  ...
  Contractor& contractor=Contractor::theContractor();
  complex_t traceRho=getEVRho(rho,projMPO);
  if(abs(traceRho)<1E-6){
    cout<<"** WARNING: Non physical state detected, trace(Proj rho)="<<traceRho<<endl;
    result=-1;    
  }
  ...
  for(int k=0;k<L;k++){
    aux.setA(k,op1_); // operator on site k
    complex_t valK=contractor.contract(rho,projMPO,aux);
    os<<real(valK/traceRho)<<"\t";
    aux.setA(k,basic); // operator removed
    if(abs(real(valK/traceRho))>1){
      cout<<"** WARNING: Non physical state detected, site "<<k<<" shows EV="<<real(valK/traceRho)
...
void doubleMPO(const MPO& mpo,MPO & doubleMPO,bool adjoint){
  int L=mpo.getLength();
  doubleMPO.initLength(L);
  for(int pos=0;pos<L;pos++){
    const mwArray aux=mpo.getOpData(pos);
    mwArray aux2(aux);
    if(adjoint){
      aux2.permute(Indices(3,2,1,4),true); // now is the Op for O+
    }
    // for tht double op, I transpose aux2
    aux2.permute(Indices(3,2,1,4));
    doubleMPO.setOp(pos,new DoubleOperator(aux,aux2),true);
  }
}
