...
/** 
    Program coherentMinusProps: Compute the steady state of a certain
    Liouvillian
    as in \class <CoherentDissipationLiouvillian>,
    with g, gamma given as input parameters.
    In this case, the relative phase is epsilon=-1

    To compile: make coherentM
...
/** Prepare the MPs corresponding to the Id operator */
void constructId(MPS& result,int L,int d);
...
int main(int argc,const char* argv[]){
  ...
  // If specified, a term -penalty |I><I| will be added to the minimization
  double tracePen=props.getDoubleProperty("tracePenalty");
  if(tracePen<0) tracePen=0;
  ...
  CoherentDissipationLiouvillian model(L,scal*g,scal*gamma,-1.*ONE_c);
  ...
  MPS tracingMPS(L,1,d*d); // The MPS that implements the trace (i.e. identity in MPS format)
  constructId(tracingMPS,L,d);
  ...
  if(wur>0){
	...
	if(tracePen==0){
	  ...
	  vector<double> penalty(1,tracePen);
	  vector<MPS*> projector;
	  projector.push_back(&tracingMPS);
	  contractor.sweepPartWithProjectorPenalty(pos,pos+1,LdaggerL,D,penalty,projector,myGS,offset); // one right
	  contractor.sweepPartWithProjectorPenalty(L-1-pos,L-1-pos-1,LdaggerL,D,penalty,projector,myGS,offset); // one left
	}
      }
      lambda=real(contractor.contract(myGS,LdaggerL,myGS));
      complex_t trR=contractor.contract(tracingMPS,myGS);
      complex_t trRR=contractor.contract(myGS,myGS);
      cout<<"After symmetric sweep nr "<<pp+1<<"trace="<<trR<<", trace(rho^2)="<<trRR<<", energy value "<<lambda<<endl;
    }
    myGS.gaugeCond('R',1);
    myGS.gaugeCond('L',1);
  }
  ...
  if(tracePen==0){
    ...
    vector<MPS*> projector;
    projector.push_back(&tracingMPS);
    contractor.findGroundStateWithProjectorPenalty(LdaggerL,D,penalty,projector,
						   &lambda,myGS,offset);
  }
  clock_t end=clock();
  ...
  *out1<<"% trace(rho)="<<contractor.contract(myGS,tracingMPS)<<endl;
  *out1<<"% trace(rho*rho)="<<contractor.contract(myGS,myGS)<<endl;
  ...
  *out2<<"% Every line starts with an integer: 0 for the lowest eigenvalue "
  ...
  if(!jobsdir.empty()){
	...
	cout<<"Error happened with D>1=> should try a different initial state!"<<endl;
	exit(333);
      }
    }
    int mpsSize=ceil(16*L*nextD*nextD*(d*d+2)*1E-6)+200;
    stringstream jobName;
    jobName<<"srm."<<L<<"."<<g<<"."<<gamma<<"."<<D;
