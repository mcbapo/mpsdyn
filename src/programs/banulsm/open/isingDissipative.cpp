...
/** 
    Program isingDissipative: Find the steady state of an Ising chain
    with local dissipation, as specified in the model
    \class<IsingLocalDissipLiouvillian>

    The program requires a first argument with the name of a
    Properties file. For instance, projects/banulsm/config/dissipIsing.conf
    (vu also an empty file would work)

    To replace some of the values (or provide missing ones) in the
    file by a command line option, the additional arguments have to be
    of the form -propName=propValue
    Accepted properties are:

    + L (int) length of the chain
    + D (int) maximum bond dimension
    + V (double) parameter \f$V\f$ of the Hamiltonian
    + Omega (double) parameter \f$\Omega\f$ of the Hamiltonian
    + Delta (double) parameter \f$\Delta\f$ of the Hamiltonian
    + gamma (double) parameter \f$\gamma\f$ of the dissipation
    ...
    + mpsdir (string) directory where to store such MPS files (default .)
    + oldmpsfile [OPTIONAL] (string) name of another (binary) file, containing 
                    an earlier MPS to be used as starting point. If it is "auto", 
		    the program searches for automatically created files in the 
		    mpsdir
    + outdir (string) directory where to write the (automatically
                    named) file(s) containing observables.
    + wurounds [OPTIONAL] (int) Number of warm up rounds for the D=1 optimization
    + mode [OPTIONAL] (int) initial product state to be used:
                 0 (default) - prod |00>
		 1 - prod |10> (Z- for d=2)
		 2 - prod |01+10>
		 3 - prod |00+11>
		 ...
		 5 - random
    + saveTmp [OPTIONAL] (int) Frequency (in nr. rounds) to save intermediate 
                    results during the optimization.
    + allDs [OPTIONAL] (vector of int) List of all values of D we want to try (check example properties)
    + [OPTIONAL]
    + [OPTIONAL]
...
void writeHeader(ofstream& out,
		 int L,double V,double Omega,double Delta,double gamma,
		 double lambda0,double purity,
...
void makeHermitian(MPS& mps,int pos,int d);
 ...
 double computePurity(const MPS& rho);
...
int main(int argc,const char* argv[]){
  ...
  double Omega=props.getDoubleProperty("Omega");
  ...
  string newMPSdir=props.getProperty("mpsdir");
  if(newMPSdir.empty()) newMPSdir=".";
  ...
  if(oldMPSfile=="auto"){
    ...
    while(it-->allDs.begin()&&!found){
      ...
      if(*it<=D){
	char oldFile[MAXLEN];
	sprintf(oldFile,"MPS_%d_%1.2f_%1.2f_%1.2f_%1.2f_%d.dat",L,V,Omega,Delta,gamma,*it);
  ...
  // one line)
  sprintf(fileNameC,"dissipIsingSingle_%d_%1.2f_%1.2f_%1.2f_%1.2f_%d.txt",L,V,Omega,Delta,gamma,D);
  ...
  sprintf(fileNameC,"dissipIsingDouble_%d_%1.2f_%1.2f_%1.2f_%1.2f_%d.txt",L,V,Omega,Delta,gamma,D);
  const string out2bodyfile=outdir+"/"+fileNameC;


  // Also the name of the MPS file
  sprintf(fileNameC,"MPS_%d_%1.2f_%1.2f_%1.2f_%1.2f_%d.dat",L,V,Omega,Delta,gamma,D);
  const string newMPSfile=newMPSdir+"/"+fileNameC;
  const string tmpMPSfile=newMPSfile+".tmp"; // temporary place

  IsingLocalDissipLiouvillian model(L,V,Omega,Delta,gamma,scal);
  ...
  double purity=computePurity(myGS);
  ...
  writeHeader(*out1,L,V,Omega,Delta,gamma,lambda,purity,D,scal,cost,oldMPSfile,mode);
  ...
  *out1<<endl;
  ...
  writeHeader(*out2,L,V,Omega,Delta,gamma,lambda,purity,D,scal,cost,oldMPSfile,mode);
 ...
 double computePurity(const MPS& rho){
  ...
  double purity=real(contractor.contract(rho,rho))/traceRho;
  return purity;
 }
...
void computeTwoBodyEV(const MPS& rho,const mwArray& op1,
 ...
 void writeHeader(ofstream& out,int L,double V,double Omega,double Delta,double gamma,
		  double lambda0,double purity,
		  int D,double scal,double costT,const string oldMPSfile,int mode){
  out<<"% L="<<L<<endl;
  out<<"% V="<<V<<endl;
