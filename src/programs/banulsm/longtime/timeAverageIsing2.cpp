...
/** timeAverageIsing simulates the time averaged evolution of a
    ...
    computed.  In this second version, the initial time T0 is
    simulated as a pure state. Only then the result is approximated by
    an MPO to start the average map. Also, the time step for Trotter
    can be different from the discretization for the average.
    ...
    \param <T0> (double) [optional] initial time from which to take the 
		   ...
		   is applied to rho.
    \param <deltaAv> (double) [optional] time interval for the average (default delta)
*/
...
int main(int argc,const char* argv[]){
  ...
  if(argc>11)
    T0=atof(argv[++cntr]);
  double deltaAv=delta;
  if(argc>12)
    deltaAv=atof(argv[++cntr]);
