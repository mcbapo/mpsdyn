...
*/
const string jobfilename(int L,int D,double E,double J,double g,double h,const string jobsdir);
/** 
Generates a file name for the MPS corresponding to a certain energy.
*/
const string mpsfilename(int L,int D,double E,double J,double g,double h,const string mpsdir);
...
void writeJob(const string jobsdir,const string exec,const string configfile,
	      int L,int D,double E,double J,double g,double h,double tol,
...
int main(int argc,const char* argv[]){
  ...
  if(firstCall){
    ...
    // Save as first state
    string gsfilename=mpsfilename(L,D,Emin,J,g,h,mpsdir);
    ...
    string excfilename=mpsfilename(L,D,Emax,J,g,h,mpsdir);
    ...
    if(nrStates>1){
      ...
      writeJob(jobsdir,argv[0],argv[1],L,D,Emin+deltaE,J,g,h,tol,Emin,Emax,nrStates,
	       mpsdir,penH,"",Dmin,Dmax,stepD,launchImmediately);
      writeJob(jobsdir,argv[0],argv[1],L,D,Emax-deltaE,J,g,h,tol,Emin,Emax,nrStates,
	       mpsdir,penH,"",Dmin,Dmax,stepD,launchImmediately);
      // now the ones in D
      if(D+stepD<=Dmax){
	writeJob(jobsdir,argv[0],argv[1],L,D+stepD,Emin,J,g,h,tol,Emin,Emax,nrStates,
		 mpsdir,penH,gsfilename,Dmin,Dmax,stepD,launchImmediately);
	writeJob(jobsdir,argv[0],argv[1],L,D+stepD,Emax,J,g,h,tol,Emin,Emax,nrStates,
		 mpsdir,penH,excfilename,Dmin,Dmax,stepD,launchImmediately);
      }
    }
    else{ // launch just one, this time knowing the edges
      writeJob(jobsdir,argv[0],argv[1],L,D,E0,J,g,h,tol,Emin,Emax,nrStates,
	       mpsdir,penH,"",Dmin,Dmax,stepD,launchImmediately);
    }
  }
  else{ // Compute just one state
    MPS gs(L,D,d);
    string mpsfile=mpsfilename(L,D,E0,J,g,h,mpsdir);    
    ...
    if(launchImmediately){
      ...
      // increase D
      if(D+stepD<=Dmax){
	writeJob(jobsdir,argv[0],argv[1],L,D+stepD,E0,J,g,h,tol,Emin,Emax,nrStates,
	...
	if(E0*nextE>=0||abs(nextE)<.5*deltaE){ // both on the same side. 
	  writeJob(jobsdir,argv[0],argv[1],L,D,nextE,J,g,h,tol,Emin,Emax,nrStates,
		   mpsdir,penH,mpsfile,Dmin,Dmax,stepD,launchImmediately);
	}    
      }  
    }
...
const string mpsfilename(int L,int D,double E,double J,double g,double h,const string mpsdir){
  stringstream s;
  s<<mpsdir<<"/mps_N"<<L<<"_J"<<J<<"_g"<<g<<"_h"<<h<<"_E"<<E<<"_D"<<D<<".dat";  
...
const string jobfilename(int L,int D,double E,double J,double g,double h,const string jobsdir){
  stringstream s;
  s<<jobsdir<<"/job_L"<<L<<"_J"<<J<<"_g"<<g<<"_h"<<h<<"_E"<<E<<"_D"<<D<<".dat";
    //  cout<<"mpsfilename returns "<<s.str()<<endl;
  return s.str();
}
...
void writeJob(const string jobsdir,const string exec,const string configfile,
  ...
  string jobfile=jobfilename(L,D,E,J,g,h,jobsdir);
  stringstream commandstr;
  commandstr<<exec;
  commandstr<<" "<<configfile;
  commandstr<<" -L="<<L<<" -J="<<J<<" -g="<<g<<" -h="<<h;
  ...
  if(!ojob->is_open()){
    ...
    *ojob<<" -N loVa.L"<<L<<".D"<<D<<".E"<<E<<".J"<<J<<".g"<<g<<".h"<<h<<" ";
    *ojob<<" -- ";
