...
// Define if left and right parts are to be combined with minus relative sign
#define RELATIVESIGN 1
...
int main(int argc,const char* argv[]){
  ...
  *out<<setprecision(10);
  ...
  double* Vis1=new double[L];
  double* Vis2=new double[L];
  ...
  for(int k=0;k<L;k++){
    ...
    Vis2[k]=A/(aux2*aux2);
    Vis0[k]=0;
    cout<<"Site "<<k<<", U="<<Uis0[k]<<",V="<<Vis1[k]<<endl;
  }

  BoseHubbardHamiltonian hBH0(L,N,tis,Uis0,muis0,Vis1);
  ...
  MPS gs1(gsP); // first one

  BoseHubbardHamiltonian hBH2(L,N,tis,Uis0,muis0,Vis2);
  const MPO& hamil2=hBH2.getHMPO();
  contractor.findGroundState(hamil2,D,&lambda,gsP);
  MPS gs2(gsP); // second one

  // Optimize the sum (with plus or minus sign)
  std::vector<const MPS*> gss;
  gss.push_back(&gs1);
  gss.push_back(&gs2);
  //gss.push_back(&gs1);
  std::vector<complex_t> coefs;
  coefs.push_back((complex_t){1/sqrt(2),0.});
#ifdef RELATIVESIGN
  coefs.push_back((complex_t){-1/sqrt(2),0.});
#else
  coefs.push_back((complex_t){1/sqrt(2),0.});
#endif
  D=2*D; // to keep both of them 
  contractor.optimizeSum(gss,coefs,gsP,D);
  gsP.gaugeCond('R',1); // normalize, just in case
  ...
  *out<<"%k\t t(k)\t U(k)\t mu(k)\t V(k)\t <n(k)>"<<endl;
  ...
  for(int k=0;k<L;k++){
    ...
    nrk=nrk/normGS;
    *out<<k<<"\t"<<tis[k]<<"\t"<<Uis0[k]<<"\t"<<muis0[k]<<"\t"<<Vis1[k]
	<<"\t"<<Vis2[k]<<"\t"<<nrk<<endl;
  ...
  *out<<"% Ntot="<<totalN<<" N^2="<<totalN2<<", Delta N="<<totalN2-totalN*totalN<<endl;
  ...
  BoseHubbardHamiltonian hBH(L,N,tis,Uis,muis,Vis0);
