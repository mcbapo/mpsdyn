...
/** nnhamiltonian reads a Hamiltonian \ref <NNHamiltonian>
    from a text file, and finds its ground state.

    Receives arguments:
    \param <infname> (char*) name of the input file
...
int main(int argc,const char* argv[]){
  ...
  *out<<"% Imaginary time configuration from "<<timefname<<endl;
  *out<<setprecision(16);
  ...
  *out<<"% D\t E(GS) \t "<<endl;
  ...
  inlog.close();
  cout<<"Read time configuration"<<endl;
  ...
  *out<<D<<"\t"<<energyGS/normGS<<endl;
  ...
  // Now test also imaginary time evolution
  MPS evolS(L,D,dims);
  evolS.setRandomState(); // the initial state, random -> not to the GS!!
  evolS.gaugeCond('R',1);
  evolS.gaugeCond('L',1);
  ...
  // myH.getExponentialMPOeven(expHe,(complex_t){-deltaR,0});
  //myH.getExponentialMPOeven(expHe_2,(complex_t){-deltaR/2,0});
  //myH.getExponentialMPOodd(expHo,(complex_t){-deltaR,0});
  ...
  expHe.exportMPOtext("expHe.txt");
  expHe_2.exportMPOtext("expHe2.txt");
  expHo.exportMPOtext("expHo.txt");

  int cnt=0;  double time=0.;
  *out<<"% %%%%%% Result of imaginary time evolution %%%% "<<endl;
  *out<<"% Nr step \t delta \t time \t Energy \t Overlap with GS"<<endl;
  ...
  while(cnt<M){
    ...
    evolS.gaugeCond('R',1);
    double energy=real(contractor.contract(evolS,hamil,evolS));
    *out<<cnt<<"\t"<<deltaR<<"\t"<<time
	<<"\t"<<energy
	<<"\t"<<contractor.contract(evolS,gsP)<<endl;
    cout<<"After (imaginary) time evolution step "<<cnt
	<<", E="<<energy<<" (Egs="<<energyGS/normGS<<")"<<endl;
