/**
   \file flatIsing.cpp
   Evolve Ising ground state after a local perturbation, to compare to
   results from nnpert.cpp
   Compute also entropy and spectrum of the rdm at the center.
...
/** flatIsing runs the findGroundState routine with the MPO for the
    ...
    \param <outfname3> (char*) name of the output file for the eigenvalues
...
int main(int argc,const char* argv[]){
   ...
   *out<<endl;
   ...
   complex_t sigmaZ_[]={ONE_c,ZERO_c,ZERO_c,-1*ONE_c};
   mwArray sigZ(Indices(d,d),sigmaZ_);
   // On two central sites
   //   applyPerturbation(gs,state,center);
   state.applyLocalOperator(center,sigZ);
   ...
   *out3<<"% M\t t\t E \t <S(pos)>"<<endl;
  }
  else
    out3=new ofstream(outfname3,ios::app);
  if(!out3->is_open()){
    cout<<"Error: impossible to open file "<<outfname3<<
      " for output"<<endl;
    exit(1);
  }
   ...
   while(cnt<M){
	 ...
	 *out2<<entr<<"\t";
       ...
       *out3<<cnt<<"\t"<<time<<"\t";
       mwArray rdm2=contractor.contract(state,state,center,'L');
       vector<complex_t> vals;mwArray U;
       wrapper::eig(rdm2,vals,U,false);
