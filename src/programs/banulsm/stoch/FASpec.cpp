...
const string jobfilename(int L,int D,double sL,double c,bool pbc,const string jobsdir);
...
void mpoNN(MPO& mpo,int L,bool pbc=0);
...
int main(int argc,const char* argv[]){
  ...
  while(lval<=nlev-1){
    ...
    // Unless ignoreTmp is set, first check if a temporary file for this state exists in the initmpsdir directory
	...
	cout<<"There is no tmp file (or ignoring) for same D: "<<tmpfilename<<endl;    
	...
	contractor.findGroundState(hamil,D,&lambda,*exc,0.,knr,tmpfilename,freqSave);
      ...
      // Orthogonalize to the others!
      // *****
      const vector<double> penalties(lval,100);
      vector<const MPS*> prevLev;
      for(int lk=0;lk<lval;lk++) prevLev.push_back(levels[lk]);
      contractor.orthogonalize(*exc,D,prevLev,penalties);
      if(lval>=2&&L>=100){
  ...
  if(reorder){
	...
	exit(1);
      ...
      *out<<setprecision(15);
      *out<<"% Spectrum of FA model L="<<L<<", D="<<D<<endl;
      *out<<"% sL="<<sL<<endl;
      *out<<"% nlev\t <H>\t<H^2>\t <n(0)>\t <n(1)>\t ..."<<endl;
      out->close();delete out;
      if(computePols){
  ...
  if(jobsdir.length()>0&&D<maxD){
    ...
    if(maxOverlap>0) commandstr<<" -maxOverlap="<<maxOverlap;
    string jobfile=jobfilename(L,newD,sL,c,pbc,jobsdir);
    ...
    if(!ojob->is_open()){
      ...
      if(newD>=100) nPar=10;
      if(newD>=200) nPar=12;
      if(L<=40) nPar=2; // only for recomputing nn
      if(newD<20) nPar=1; // 
...
const string jobfilename(int L,int D,double sL,double c,bool pbc,const string jobsdir){
  ...
  str<<jobsdir<<"/job_fa";
  if(pbc) str<<"PBC";
  str<<"_L"<<L<<"_sL"<<sL<<"_c"<<c<<"_D"<<D;
  str<<".dat";
  //  cout<<"mpsfilename returns "<<s.str()<<endl;
  return str.str();
}

void mpoNN(MPO& mpo,int L,bool pbc){
  mpo.initLength(L);
  int d=2;
  ...
  for(int k=0;k<L;k++){
    ...
    if(k<L-1){
      ...
      if(pbc) // first of the pair (which looks as second)
	C.setElement(ONE_c,Indices(2,Dr-1,1)); // 1st of pair
    }
    if(k!=0){
      C.setElement(ONE_c,Indices(Dl-1,Dr-1,0)); // after everything
      C.setElement(ONE_c,Indices(1,Dr-1,1)); // 2nd of pair
    }
    else if(pbc) C.setElement(ONE_c,Indices(0,2,1)); // 2nd of pair, which appears first
