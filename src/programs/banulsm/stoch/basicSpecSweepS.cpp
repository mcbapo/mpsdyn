...
const string outputfile(int L,int D,double s,double c,const string outdir);
const string polXfile(int L,int D,double s,double c,const string outdir);
...
/** basicSpec tries to find the ground state and excitations of the stochastic Hamiltonian
    ...
    This version prepares jobs to sweep over a range of s values, and increase the bond dimension.
...
int main(int argc,const char* argv[]){
  ...
  double incrs=props.getDoubleProperty("deltas");
  double s0=props.getDoubleProperty("sínit");
  double s1=props.getDoubleProperty("sfin");
  ...
  double s=s0;
  while((incrs>0&&s<=s1)||(incrs<0&&s>=s1)){
    const outfname=outputfile(L,D,d,c,outdir);
    const polfname=polXfile(L,D,d,c,outdir);
