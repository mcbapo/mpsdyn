...
int main(int argc,const char* argv[]){
  ...
  *out<<"% E0="<<lambda<<endl;
  ...
  // Now I get the Sz=0 projector
  MPO PSz(L),hamilSz(L);
  SpinMPO::getProjectorTotalSz(L,d,PSz);
  const MPO* oprsH[3]={&PSz,&hamil,&PSz}; 
  MPO::join(3,oprsH,hamilSz);
  ...
  for(int l=0;l<=nLev;l++){
    if(l>0){ // compute one new excitation)
      ...
      double lambda1=0.;
      contractor.findNextExcitedState(hamilSz,D1,levels,&lambda1,*exc);
