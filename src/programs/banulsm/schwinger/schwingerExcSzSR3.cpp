...
int main(int argc,const char* argv[]){
  ...
  if(filestate.empty()){
    cout<<"No previous MPS result used"<<endl;
  }
  ...
  *out<<"% <condensate>="<<real(fC)<<endl;
  ...
  SchwingerHamiltonianSz hSchOrigAux(L,mu,x,L0,alpha,zpen,-estimE,nu,gweight);
  ...
  if(filestate.empty())
    ...
    gs2->importMPS(filestate.data());
  gs2->increaseBondDimension(Ds[2]);
  contractor.setEigenSolver(primme);
  ...
  for(int l=0;l<nLev+1;l++){
    if(l>1){ // compute one new excitation of hamil2
      int D1=Ds[l-1];
      ...
      contractor.findNextExcitedState(hamil2,D1,levelsExc,&lambda1,*exc,offset2);
