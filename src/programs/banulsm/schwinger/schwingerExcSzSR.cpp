...
/** SchwingerExcSzSR tries to find the scalar mass gap by first
    finding the MPS approximation to the ground state, then finding
    another one for the first excited state, but with a penalty term
    in the H which raises the energy of states with negative sign of
    the SR operator (shift times sigmax), together with the usual
    penalty for Sz!=0. A fixed number of (MPS approximated)
    eigenstates is computed, and for each of them the expectation
    ...
    energies. To make the calculation more efficient, no more
    operators are computed, neither the overlaps between eigenvectors.
...
int main(int argc,const char* argv[]){
  ...
  double lambda=0.;
  SchwingerHamiltonianSzSR hSch(L,mu,x,L0,alpha,zpen,lamb,offset,nu,gweight);
  ...
  //  exit(1);
  contractor.findGroundState(hamilOrig,D0,&lambda,gs); // the GS is the good one
  ...
  *out<<"% E0="<<lambda<<endl;
  ...
  MPO SRmpo(L);
  hSch.constructShiftRotateMPO(SRmpo,false);
