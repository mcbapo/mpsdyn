...
/** SchwingerExcSzSR4 tries to find the scalar mass gap in two steps:
    1) first, it finds the MPS approximation to the ground state, then
    another one for the first excited state (vector).

    2) Using the estimated energy of the scalar state Es (provided as
    input parameter), excited states are searched close to that value.
    Several excited states are targeted and the first one with the
    proper label will be identified with the scalar state.
...
int main(int argc,const char* argv[]){
  ...
  contractor.setEigenSolver(primme_arnoldi);
  int d=2;
  MPS gs(L,D0,d);
  ...
  // To check!!
  //  hamil.exportForMatlab("schwHamilSR.m");
  //  exit(1);
  contractor.findGroundState(hamilOrig,D0,&lambda,gs);
  ...
  *out<<setprecision(10);
  *out<<"% E0="<<lambda-real(zpen*contractor.contract(gs,Sz2mpo,gs))-offset<<endl;
  ...
  MPS* vec=new MPS(L,D1,d);
  vec->setRandomState(); // the initial state, random
  double lambda1=0.;
  contractor.findNextExcitedState(hamilOrig,D1,levels,&lambda1,*vec);
  cout<<"First excited (vector) state found with eigenvalue "<<lambda1<<"=> mass gap="<<lambda1-lambda<<endl;
  levels.push_back(vec);
  // Estimate of the scalar mass (around 2*mv)
  //  double estimE=lambda1+.8*(lambda1-lambda);
  cout<<"Using estimation of the scalar energy "<<estimE<<" (gap~"<<estimE-lambda<<")"<<endl;
  // New auxiliary Hamiltonian with an offset
  ...
  MPS* gs2=new MPS(L,Ds[2],d);
  gs2->setRandomState();
  //*gs2=*vec;
  //gs2->perturb(.2);
  contractor.setEigenSolver(primme);
  //  contractor.setEigTol(1E-10);
  //  contractor.setEigenSolver(primme_JDQR); // seems to work better for squared things
  //  contractor.setEigenSolver(fulleig);
  //  contractor.setEigenSolver(primme_arnoldi);
  //  contractor.findGroundState(hamil2,Ds[2],&lambda1,*gs2,offset2);
  contractor.setConvTol(1E-5);
  contractor.findClosestEigenstate(hamilOrig,Ds[2],&lambda1,*gs2,estimE+offset);
  ...
  for(int l=0;l<=nLev+1;l++){
    if(l>2){ // compute one new excitation of hamil2
      ...
      exc->setRandomState(); // the initial state, random
       //contractor.findNextExcitedState(hamilOrig,D1,levelsExc,&lambda1,*exc,offset2);
      contractor.findNextClosestEigenstate(hamilOrig,D1,levelsExc,&lambda1,*exc,estimE+offset);
