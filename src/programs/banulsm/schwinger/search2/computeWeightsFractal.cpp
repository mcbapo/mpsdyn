...
/** This program should read the GS computed by searchOrder and
    ...
    Back to projecting the beginning of the chain.
...
int main(int argc,const char* argv[]){
  ...
  // W_001011 , W_0011, W_01 , W_10 , W_100011 , W_1001, W_1010, W_10110, W_110, W_11100
  ...
  if(!app){
	...
	*out<<"P01_000\tP01_001\tP01_01\tP01_10\tP01_110\tP01_111\t";
	*out<<"P00_01_11\tP00_10_11\tP10_00_11\tP01_00_11"<<endl;
      }
      else{
	// W_001011 , W_0011, W_01 , W_10 , W_100011 , W_1001, W_1010, W_10110, W_110, W_11100
	*out<<"P000111\tP0011\tP10\tP01\tP010011\tP0110\tP0101\t";
	*out<<"P011100\tP011110\tP011101\tP1100\tP1110\tP1101\t";
	*out<<"P110100\tP110110\t";
	*out<<endl;
      }
  }
  ...
  *outH<<setprecision(10);
  ...
  while(!done){
    ...
    if(!newDec){
      ...
      //int p0(0),p1(1),p2(2); // the first three
      // 000
      ...
      // Compute a few extra ones
      //      int p5(5);    
      proj.setOp(p0,&proj0,false); 
      proj.setOp(p1,&proj0,false);
      proj.setOp(p2,&proj0,false); 
      proj.setOp(p3,&proj1,false); 
      proj.setOp(p4,&proj1,false); 
      proj.setOp(p5,&proj1,false); 
      complex_t P00_01_11=contractor.contract(state,proj,state);
      proj.setOp(p2,&proj1,false); 
      proj.setOp(p3,&proj0,false); 
      complex_t P00_10_11=contractor.contract(state,proj,state);
      proj.setOp(p2,&proj0,false); 
      proj.setOp(p3,&proj0,false); 
      proj.setOp(p1,&proj1,false); 
      proj.setOp(p0,&proj0,false);
      complex_t P10_00_11=contractor.contract(state,proj,state);
      proj.setOp(p0,&proj1,false); 
      proj.setOp(p1,&proj0,false);
      complex_t P01_00_11=contractor.contract(state,proj,state);
    ...
    else{ // new Decomposition terms:
      //      	*out<<"P000111\tP0011\tP10\tP01\tP010011\tP0110\tP0101\t";
      //	*out<<"P011100\tP011110\tP011101\tP1100\tP1110\tP1101\t";
      //	*out<<"P110100\tP110110\t";
      proj.setOp(p0,&proj1,false); //1
      proj.setOp(p1,&proj0,false); //10
      complex_t P10=contractor.contract(state,proj,state);
      proj.setOp(p0,&proj0,false); //0
      proj.setOp(p1,&proj1,false); //01
      complex_t P01=contractor.contract(state,proj,state);
      proj.setOp(p0,&proj0,false); //0
      proj.setOp(p1,&proj0,false); //00
      proj.setOp(p2,&proj1,false); //001
      proj.setOp(p3,&proj1,false); //0011
      complex_t P00_11=contractor.contract(state,proj,state);
      proj.setOp(p0,&proj0,false); //0
      proj.setOp(p1,&proj1,false); //01
      proj.setOp(p2,&proj1,false); //011
      proj.setOp(p3,&proj0,false); //0110
      complex_t P01_10=contractor.contract(state,proj,state);
      proj.setOp(p2,&proj0,false); //010
      proj.setOp(p3,&proj1,false); //0101
      complex_t P01_01=contractor.contract(state,proj,state);
      proj.setOp(p0,&proj1,false); //1
      proj.setOp(p1,&proj1,false); //11
      proj.setOp(p2,&proj0,false); //110
      proj.setOp(p3,&proj0,false); //1100
      complex_t P11_00=contractor.contract(state,proj,state);
      proj.setOp(p2,&proj1,false); //111
      proj.setOp(p3,&proj0,false); //1110
      complex_t P11_10=contractor.contract(state,proj,state);
      proj.setOp(p2,&proj0,false); //110
      proj.setOp(p3,&proj1,false); //1101
      complex_t P11_01=contractor.contract(state,proj,state);
      proj.setOp(p4,&proj0,false); //11010
      proj.setOp(p5,&proj0,false); //110100
      complex_t P11_0100=contractor.contract(state,proj,state);
      proj.setOp(p4,&proj1,false); //11011
      proj.setOp(p5,&proj0,false); //110110
      complex_t P11_0110=contractor.contract(state,proj,state);
      proj.setOp(p0,&proj0,false); //0
      proj.setOp(p1,&proj1,false); //01
      proj.setOp(p2,&proj0,false); //010
      proj.setOp(p3,&proj0,false); //0100
      proj.setOp(p4,&proj1,false); //01001
      proj.setOp(p5,&proj1,false); //010011
      complex_t P01_0011=contractor.contract(state,proj,state);
      proj.setOp(p2,&proj1,false); //011
      proj.setOp(p3,&proj1,false); //0111
      proj.setOp(p4,&proj0,false); //01110
      proj.setOp(p5,&proj0,false); //011100
      complex_t P01_1100=contractor.contract(state,proj,state);
      proj.setOp(p4,&proj1,false); //01111
      proj.setOp(p5,&proj0,false); //011110
      complex_t P01_1110=contractor.contract(state,proj,state);
      proj.setOp(p4,&proj0,false); //01110
      proj.setOp(p5,&proj1,false); //011101
      complex_t P01_1101=contractor.contract(state,proj,state);
      proj.setOp(p0,&proj0,false); //0
      proj.setOp(p1,&proj0,false); //00
      proj.setOp(p2,&proj0,false); //000
      proj.setOp(p3,&proj1,false); //0001
      proj.setOp(p4,&proj1,false); //00011
      proj.setOp(p5,&proj1,false); //000111
      complex_t P00_0111=contractor.contract(state,proj,state);
