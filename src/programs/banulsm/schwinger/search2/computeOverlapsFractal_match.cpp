...
/** This program should read the GSs computed by searchOrder and
    ...
    In this version, I compute the elements that match theirs, which corresponds to 0<->1, but I suspect it must come from projecting onto the opposite end.
...
int main(int argc,const char* argv[]){
  ...
  if(!app){
      ...
      *out<<"%lev\tL\tx\tmu\tD\t";
      // Notice: I have now changed notation!!      
      *out<<"N2\treal(<Psi4|P01 Psi2>/sqrt(N2))\timag(<Psi4|P01 Psi2>/sqrt(N2))\t";
      *out<<"N1\treal(<Psi(4)|P10 Psi2>/sqrt(N1))\timag(<Psi(4)|P10 Psi2>/sqrt(N1))\t";
      *out<<"N9\treal(<Psi6|P0011 Psi2>/sqrt(N9))\timag(<Psi6|P0011 Psi2>/sqrt(N9))\t";
      *out<<"N10\treal(<T Psi5+|P110 Psi2>/sqrt(N10))\timag(<T Psi5+|P110 Psi2>/sqrt(N10))\t";
      *out<<"N11\treal(<Psi4|P0 T Psi3+>/sqrt(N11))\timag(<Psi4|P0 T Psi3+>/sqrt(N11))\t";
      *out<<"N8\treal(<Psi5+|P01 Psi3+>/sqrt(N8))\timag(<Psi5+|P01 Psi3+>/sqrt(N8))\t";
      *out<<"N13\treal(<Psi6|P01 Psi4>/sqrt(N13))\timag(<Psi6|P01 Psi4>/sqrt(N13))\t";
      *out<<"N14\treal(<T Psi5+|P1 Psi4>/sqrt(N14))\timag(<T Psi5+|P1 Psi4>/sqrt(N14))\t";
      *out<<"N15\treal(<Psi8|P001011 Psi2>/sqrt(N15))\timag(<Psi8|P001011 Psi2>/sqrt(N15))\t";
      *out<<"N16\treal(<T Psi7+|P11100 Psi2>/sqrt(N16))\timag(<T Psi7+|P11100 Psi2>/sqrt(N16))\t";
  ...
  *out<<setprecision(15);
  ...
  while(!done){
    ...
    *out<<normN<<"\t"<<real(overl)<<"\t"<<-imag(overl)<<"\t";
    ...
    // 15:
    cout<<"<Psi8|P001011 Psi2>"<<endl;
    pos.clear();label.clear();
    for(int k=0;k<6;k++) pos.push_back(k);
    label.push_back(1);label.push_back(1);
    label.push_back(0);
    label.push_back(1);
    label.push_back(0);label.push_back(0);
    overl=computeOverlap(normN,state8,state2,pos,label,false,false);
    *out<<normN<<"\t"<<real(overl)<<"\t"<<-imag(overl)<<"\t";

    // 16:
    cout<<"<T Psi7+|P11100 Psi2>"<<endl;
    pos.clear();label.clear();
    for(int k=0;k<5;k++) pos.push_back(k);
    label.push_back(0);label.push_back(0);label.push_back(0);
    label.push_back(1);label.push_back(1);
    overl=computeOverlap(normN,state7,state2,pos,label,true,false);
    *out<<normN<<"\t"<<real(overl)<<"\t"<<-imag(overl)<<"\t";
