...
void constructDoubleCommutatorHeis(MPO& mpo,int L,double g, double h, mwArray instances, int rowInstances);
...
int main(int argc,const char* argv[]){
  ...
  int oldD =props.getIntProperty("oldD");
  ...
  string inputMPS=mpsfilename(M,oldD,mpsdir);
  //if(initD<0) inputMPS="";
  ...
  constructDoubleCommutatorHeis(H2,M,gP,hP,instances,rowInstances);
  ...
  //  Omps.exportForMatlab("OmpsFin.m"); //export final MPS
...
void constructDoubleCommutatorHeis(MPO& mpo,int L,double g,double h, mwArray instances, int rowInstances){
  ...
  double h1 = h*real(instances.getElement(Indices(rowInstances,0)));
  //  h1 = 1.;
  ...
  double hN = h*real(instances.getElement(Indices(rowInstances,L+1)));
  //  hN = 1.;
  ...
  for(int k=1; k<L+1; k++){
    ...
    // totOps[ind].setElement(ONE_c,Indices(0,1,1));      //  sigX x I
    // totOps[ind].setElement(-1.*ONE_c,Indices(0,2,2));  //  I x sigX 
    // totOps[ind].setElement(ONE_c,Indices(0,3,3));      //  sigY x I
    // totOps[ind].setElement(-1.*ONE_c,Indices(0,4,4));  //  I x sigY
    // totOps[ind].setElement(ONE_c,Indices(0,5,5));      //  sigZ x I
    // totOps[ind].setElement(-1.*ONE_c,Indices(0,6,6));  //  I x sigZ 
    
    // totOps[ind].setElement(ONE_c,Indices(1,D-1,1));      //  sigX x I
    // totOps[ind].setElement(ONE_c,Indices(2,D-1,2));  //  I x sigX 
    // totOps[ind].setElement(ONE_c,Indices(3,D-1,3));      //  sigY x I
    // totOps[ind].setElement(ONE_c,Indices(4,D-1,4));  //  I x sigY
    // totOps[ind].setElement(ONE_c,Indices(5,D-1,5));      //  sigZ x I
    // totOps[ind].setElement(ONE_c,Indices(6,D-1,6));  //  I x sigZ 
  ...
  for(int r=0;r<L;r++){
    cout<<real(totOps[r].getElement(Indices(2,0,2,D-1)))<<",";
  }
...
void constructDoubleCommutatorIsing(MPO& mpo,int L,double g,double h){
  ...
  // edgeLd.permute(Indices(1,2,4,3));edgeLd.reshape(Indices(dd*1*D,D*dd));
  // edgeL.reshape(Indices(D*dd,1*dd*D));
  
  // mwArray el = edgeLd*edgeL; el.reshape(Indices(dd,1,D,1,dd,D));
  // el.permute(Indices(1,2,4,5,3,6));el.reshape(Indices(dd,1*1,dd,D*D));
  
  // edgeRd.permute(Indices(1,2,4,3));edgeRd.reshape(Indices(dd*D*1,D*dd));
  // edgeR.reshape(Indices(D*dd,D*dd*1));
  // mwArray er = edgeRd*edgeR; er.reshape(Indices(dd,D,1,D,dd,1));
  // er.permute(Indices(1,2,4,5,3,6));er.reshape(Indices(dd,D*D,dd,1*1));
  
  // tensors.push_back(&el); 
  
  // mwArray Cd;
  // for(int k=1;k<L-1;k++){
  //   Cd = totOps[k]; 
  //   totOps[k].permute(Indices(1,2,3,4));
  //   totOps[k].reshape(Indices(dd,D*dd*D));Cd.reshape(Indices(dd*D*D,dd));
  //   totOps[k] = Cd*totOps[k]; totOps[k].reshape(Indices(dd,D*D,D,dd,D));
  //   totOps[k].permute(Indices(1,2,4,3,5)); totOps[k].reshape(Indices(dd,D*D,dd,D*D));
  //   tensors.push_back(&totOps[k]);
  // }
  // tensors.push_back(&er);
  

  // mpo.setOperatorArray(tensors);
