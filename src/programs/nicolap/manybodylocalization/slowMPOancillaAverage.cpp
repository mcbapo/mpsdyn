...
void constructDoubleCommutatorHeisembergAncillaAverage(MPO& mpo,int L,double J1x,double J1y, double J1z,double J2x, double J2y, double J2z, double Bond);
...
int main(int argc,const char* argv[]){
  ...
  constructDoubleCommutatorHeisembergAncillaAverage(H2,M,J1x,J1y,J1z,J2x,J2y,J2z,hP);
...
void constructDoubleCommutatorHeisembergAncillaAverage(MPO& mpo,int L,double J1x,double J1y, double J1z,double J2x, double J2y, double J2z, double Bond){
  ...
  mwArray aver(Indices(d,d,d));
  aver.setElement(ONE_c/sqrt(2.),Indices(0,0,0));
  aver.setElement(ONE_c/sqrt(2.),Indices(1,1,1));
  aver.reshape(Indices(dd,d));
  ...
  CoL.multiplyRight(aver);
  
  CoL.reshape(Indices(D,1,dd*d));CoL.permute(Indices(1,3,2));  //D dd*d 1

  Co.reshape(Indices(D*D,nrOps));Co.multiplyRight(Z);
  Co.reshape(Indices(D*D*dd,dd));
  Co.multiplyRight(aver);

  Co.reshape(Indices(D,D,dd*d,1));Co.permute(Indices(3,1,4,2));//dd*d D 1 D

  int d5 = dd*dd*d;

  mwArray tmp(Co);
  tmp.permute(Indices(2,1,3,4));
  tmp.reshape(Indices(D,dd*d*1*D));
  ...
  tmp=CoL; tmp.reshape(Indices(D,dd*d*1));
  CeL.reshape(Indices(dd*D*1,D));
  CeL.multiplyRight(tmp);
  CeL.reshape(Indices(dd,D,1,dd*d,1)); CeL.permute(Indices(1,4,2,3,5));
  ...
  tmp=Co;tmp.reshape(Indices(dd*d*D*1,D));
  CeL.permute(Indices(2,1)); // put first the index for 2nd MPO
  mwArray edgeR=tmp*CeL;edgeR.reshape(Indices(dd*d,D,1,D));
  edgeR.permute(Indices(1,4,2,3));edgeR.reshape(Indices(dd*d*D,D,1,1)); // ddD D dd 1
  ...
  mwArray edgeRdo(Cdo);edgeRdo.reshape(Indices(1,D,dd*d*D,1));
