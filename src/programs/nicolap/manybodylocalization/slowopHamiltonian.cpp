...
const string mpsfilename(int step,const string mpsdir,const string which);
...
int main(int argc,const char* argv[]){
  ...
  string outfnameG=props.getProperty("outgl");
  string outfnameL=props.getProperty("outfl");
  ...
  for(int k=0;k<=M+1;k++){
    ...
    string outputMPS=mpsfilename(k,mpsdir,"state");   
    state.exportMPS(outputMPS.data());
    ...
    cout<<"Length State: "<<lenState<<" length Op: "<<EvenH.size()<<endl;
    {
      MPS aux(state);
    ...
    //  ###### IMPOSE GAUGE CONDITION??? ####
    //state.gaugeCond('R',1);state.gaugeCond('L',1); 
    ...
    string outputMPS=mpsfilename(k,mpsdir,"stateL");   
    stateL.exportMPS(outputMPS.data());
  ...
  ofstream* out;
  out=new ofstream(outfnameL.data(),ios::app);
  if(!out->is_open()){
  ...
  *out<<"# M\t min(lambda)"<<endl;
  ...
  for(int m=1;m<=M;m++){
    ...
    for(int i=0;i<=m;i++){
      ...
      inputMPS=mpsfilename(i,mpsdir,"state");
      iPart.importMPS(inputMPS.data());

      while(iPart.getLength() != H2.getLength()){ 
	extendMPSwithId(jPart);
      }
      
      for(int j=0;j<i;j++){

	inputMPS=mpsfilename(j,mpsdir,"stateL");
	jPart.importMPS(inputMPS.data());

	while(iPart.getLength() != iPart.getLength()){ 
	  extendMPSwithId(jPart);
	}
	complex_t matEle=contractor.contract(iPart,H2,jPart);
	complex_t matNorm=contractor.contract(iPart,jPart);
	X.setElement((matEle),Indices(i,j));
	if(traceless){
	  Y.setElement((matNorm),Indices(i,j));
...
const string mpsfilename(int step,const string mpsdir,const string which){
  stringstream s;
  s<<mpsdir<<"/"<<which<<"/MPS"<<"_k"<<step<<".dat";  
