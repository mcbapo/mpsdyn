...
int main(int argc,const char* argv[]){
  ...
  int initialPosition = props.getIntProperty("initialPosition");
  ...
  double decay=1.;
  for(int k=0;k<M;k++){
    decay = 1 + 1./(abs(k-initialPosition) + 1);
    idPen.setA(k,reshape(decay*identityMatrix(d),Indices(d*d,1,1)));
  }
  ...
  cout<<"Lambda before partial sweeping = "<<lambda/norm<<" Norm = "<<norm<<endl;



  if ( initialPosition ) {
    cout<<"Initial position: "<<initialPosition<<endl;
    contractor.sweepPartWithProjPenalty(initialPosition,0,H2,D,1.*pen, 
					idPen, Omps, 0., 1);
    cout<<"Sweep from "<<initialPosition<<" to 0, done!"<<endl;
    contractor.sweepPartWithProjPenalty(initialPosition,M-2,
					H2,D,1.*pen, idPen, Omps, 0., 1);
    cout<<"Sweep from "<<initialPosition<<" to end, done!"<<endl;
  }



  lambda=real(contractor.contract(Omps,H2,Omps));
  cout<<"Lambda after partial sweeping = "<<lambda<<endl;

  contractor.findGroundStateWithProjectorPenalty(H2,D,penalty, convergenceTools,
						 orthog,&lambda,Omps,0.,1,tmpoutfile);
