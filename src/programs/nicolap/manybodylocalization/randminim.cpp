...
int main(int argc,const char* argv[]){
  ...
  int inputD=props.getIntProperty("inputD");
  ...
  if(initD<0) inputMPS="";
  string nametmp=mpsfilename(M,inputD,mpsdir);
   if(inputD<0) nametmp="noFile";
  const string tmpfilename = nametmp + ".tmp";
  ...
  //third element: lenght of the interval to check
  ...
  if(file_exists(tmpfilename)){
    cout<<"There is a tmp file for same D: "<<mpsdir<<"/"<<tmpfilename<<endl;
  ...
  contractor.findGroundStateWithProjectorPenalty(H2,D,penalty, convergenceTools,orthog,&lambda,Omps,0.,1,tmpfilename);
  clock_t endclock = clock();
  time_t endtime = time(0);
  double time = difftime(endtime, startime) * 1000.0;
  double clock = (double) (endclock-startclock) / CLOCKS_PER_SEC * 1000.0;
  cout <<"time algorithm: "<<time<<endl;
  cout<<"clock algorithm: "<<clock<<endl;
  ...
  *out<<D<<"\t"<<lambda<<"\t"<<M<<"\t"<<computeTrace(Omps)<<endl;
  ...
  delete out;
  int newD;
  newD=D+20; // in general
  // other cases
  if(D<20){
    newD=5+D;
  }

  if(abs((lambda-oldLambda)/(lambda))>=tolD&&D<maxD){ 
    ...
    const string jobfile=jobfilename(M,newD,hP,jobsdir);
    ...
    if(!ojob->is_open()){
      ...
      *ojob<<"msub ";
      // //if(D+20>60) *ojob<<" -P 4";
      *ojob<<" -N rand.M"<<M<<".D"<<newD<<".h"<<hP<<".inst"<<rowInstances;
      *ojob<<" -q normal ";
    ...
    const string jobfile=jobfilename(M,D,hP,jobsdir);
    ofstream* ojob=new ofstream(jobfile.data());
    if(!ojob->is_open()){
      cout<<"WARNING!!!! Could not open the JOB file "<<jobfile<<". Launch by hand with the following command:"<<endl;
    }else{
