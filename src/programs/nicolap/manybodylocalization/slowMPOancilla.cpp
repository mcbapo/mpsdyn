...
void constructDoubleCommutatorHeisembergAncilla(MPO& mpo,int L,double J1x,double J1y, double J1z,double J2x, double J2y, double J2z,double Bond);
...
int main(int argc,const char* argv[]){
  ...
  //double Bond=.5;
  constructDoubleCommutatorHeisembergAncilla(H2,M,J1x,J1y,J1z,J2x,J2y,J2z,hP);
...
void constructDoubleCommutatorHeisembergAncilla(MPO& mpo,int L,double J1x,double J1y, double J1z,double J2x, double J2y, double J2z, double Bond){
  ...
  Co.reshape(Indices(D,D,dd,dd));Co.permute(Indices(3,1,4,2));//dd D dd D
  ...
  mwArray tmp(Co);
  tmp.permute(Indices(1,2,4,3)); tmp.reshape(Indices(dd*D*D,dd));
  tmp.multiplyRight(id2);
  tmp.reshape(Indices(dd,D,D,1));tmp.permute(Indices(2,1,4,3));
  ...
  tmp=Co;tmp.reshape(Indices(dd*D*dd,D));
  CeL.permute(Indices(2,1)); // put first the index for 2nd MPO
  mwArray edgeR=tmp*CeL;edgeR.reshape(Indices(dd,D,dd,D));
  ...
  mwArray edgeRdo(Cdo);edgeRdo.reshape(Indices(dd,D,dd*D,1));
