/**
   \file SHGIGaussLawAngular_test.cpp
...
int main(int argc,const char* argv[]){
  ...
  mps.gaugeCond('R',1);
  mps.gaugeCond('L',1);
  SHGIGaussLawAngular Hsngi(N,d,D,mu,x,e,lam);
  ...
  convert.clear();
  convert << mu;
  string GS_without_ending;
  GS_without_ending = GS_name+"mu"+convert.str(); 
  ...
  time(&end_local);
  string command = "cp " + GS_name + " /ptmp/mpq/skuehn/MD5test/Filesystemtest/" + GS_without_ending + "_kopie.dat";
  string new_file = "/ptmp/mpq/skuehn/MD5test/Filesystemtest/" + GS_without_ending + "_kopie.dat";
  cout << endl << "Copy command " << command << endl;
  system(command.c_str());
  cout << "New File " << new_file << endl << endl;
  bool outcome_md5;
  outcome_md5 = md5_file_compare((char*) GS_name.c_str(),(char*) new_file.c_str());
   
  cout << "Outcome MD5-comparison " <<  outcome_md5 << endl;
  if(!outcome_md5)
  {
    cout << "MD-COMPARISON GAVE AN ERROR" << endl;
  ...
  md5sum = get_md5sum(GS_name.c_str());
  cout << "MD5-sum is " ;
