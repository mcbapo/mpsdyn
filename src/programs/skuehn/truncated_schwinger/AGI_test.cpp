/**
   \file AGI_test.cpp
   In this version the exponential of the link fields is approximated by the taylor expansion up to a given order and therefore the level auf gauge invariance can be controlled
  ...
  \param <order> (unsigned int) Order up to which the exponential of the gauge fields is approximated
...
#define FILENAME "SchwingerAGI.txt"
...
int main(int argc,const char* argv[]){
  ...
  if(argc != NUM_OF_PARAMS + 1){
    ...
    cout << "N: " << "Number of spins" << endl
	 ...
	 << "o: " << "Order up to which the exponential is approximated" << endl;
    return -1;
  }
  ...
  unsigned int order=atoi(argv[++cntr]);
  ...
  mps.gaugeCond('R',1);
  mps.gaugeCond('L',1);
  SchwingerHamiltonianAGIGaussLaw Hsngi(N,d,D,mu,x,e,lam,order);
