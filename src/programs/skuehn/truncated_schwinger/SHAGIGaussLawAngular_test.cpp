/**
   \file SHAGIGaussLawAngular_test.cpp
   Full gauge invariant version of the Schwinger Model yet with truncated bosonic dimension (the link fileds are APPROXIMATED up to the desired order of the exponential)
  ...
  \param <ig> (void) Variable to determine if inital guess for the ground state should be read from HDD (optional)
  \param <ac> (double) Accuracy for the contractor up to which the ground state should be calculated (optional)
  \param <O> (unsigned int) Order up to which the exponential of the link fields is approximated by its series representation, the default value is 1 (optional)
...
int main(int argc,const char* argv[]){
  ...
  if(argc < NUM_OF_PARAMS + 1){
    ...
    cout << "N: " << "Number of spins" << endl
	 ...
	 << "O: " << "Order up to which the exponential is approximated, by default it is 1 (optional)" << endl;
    return -1;
  }
  ...
  SHAGIGaussLawAngular Hsngi(N,d,D,mu,x,e,lam,exp_order);
