...
int main(int argc,const char* argv[]){
  ...
  if(evolution_order==2)
  {
    ...
    energy_file.open("KS_efile2.txt");
  ...
  if(evolution_order==3)
  {
    ...
    energy_file.open("KS_efile3.txt");
    energy_file <<"#Time\t"
                <<"Energy\t"
		<<"Norm\t"
		<<"Penalty\t"
  ...
  if(evolution_order==4)
  {
    ...
    energy_file.open("KS_efile4.txt");
    energy_file <<"#Time\t"
                <<"Energy\t"
		<<"Norm\t"
		<<"Penalty\t"
  ...
  if(evolution_order==5)
  {
    ...
    energy_file.open("KS_efile5.txt");
    energy_file <<"#Time\t"
                <<"Energy\t"
		<<"Norm\t"
		<<"Penalty\t"
  ...
  if(evolution_order==12)
  {
    ...
    energy_file.open("KS_efile12.txt");
    energy_file <<"#Time\t"
                <<"Energy\t"
		<<"Norm\t"
		<<"Penalty\t"
	...
	if((((i+1)%BU_RESOLUTION)==0)&&((i+1)!=num_of_steps))
	{
  ...
  if(evolution_order==13)
  {
    ...
    energy_file.open("KS_efile13.txt");
    energy_file <<"#Time\t"
                <<"Energy\t"
		<<"Norm\t"
		<<"Penalty\t"
	...
	cout << "Step:             " << i+1 << endl
	     ...
	     << "Truncation Error: " << errtot << endl
	     << "Worst Error:      " << total_max_err << endl << endl;
	...
	if(take_data_flag)
	{
	  ...
	  if(get_overlap && (abs(curr_overlap)<THRESHOLD))
	  {
	    ...
	    //I write a status file here, which later on allows me to identify via script if I killed the job on purpose because of bad overlaps or if it simply died
	    //Output file
	    ofstream killfile;
	    ...
	    killfile.open(KILLFILE);
	    killfile << "Job was killed on " << asctime(timeinfo) << " because the overlap was less then the given threshold" << THRESHOLD << endl;	    
	    killfile.close();    
	    exit(666);
	  }
