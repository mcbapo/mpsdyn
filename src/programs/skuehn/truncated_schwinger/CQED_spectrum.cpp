/**
   \file CQED_spectrum.cpp
   Realisation of a simulation of the fundamental Hamiltonian for 1+1d cQED as proposed in Ignacios review
   This is basically the same file as CQED_test.cpp, however this one is optimzed to explore the spectrum over a certain range to find out the energy-levels of various excited states and so on
...
int main(int argc,const char* argv[]){
  ...
  //Get the Hamiltonian and Penalty
  ...
  for (double x=xs; x<=xe; x+=dx)
  {
    ...
    if(file_exists("SpectrumCQED.txt"))
    {
      spectrumfile.open("SpectrumCQED.txt",ios_base::app);
    }
    else
    { 
      spectrumfile.open("SpectrumCQED.txt",ios_base::app);
	...
	cout << "Exstate["<<iter<<"] with energy " << contractor.contract(*(excitedstates[iter]),hamil,*(excitedstates[iter])) << " and penalty "  <<  contractor.contract(*(excitedstates[iter]),Penalty,*(excitedstates[iter])) <<endl;
