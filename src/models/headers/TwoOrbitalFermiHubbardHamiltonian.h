...
protected:
 ...
 public:
  ...
  /** The penalty terms can be set after construction (which only
      allows the total number of fermions). If this method is called,
      the penalties are ALL reset, and it supersedes the constructor
      parameters. If one of the penalties is zero, that term is ignored.  */
  void setPenalties(int Sz,double penaltySz,int Ntot,double penaltyNtot);
 ...
 private:
  ...
  // spin-spin correlator for type 1 and 2 (x, y)
