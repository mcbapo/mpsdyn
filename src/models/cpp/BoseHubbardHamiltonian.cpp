...
BoseHubbardHamiltonian::BoseHubbardHamiltonian(int L_,int N_,double t_,double U_,
					       int Ntarg_,double penN_,
					       double V_):
  hamil(L_),tis(1,t_),Uis(1,.5*U_),Vis(1,V_){
  L=L_;N=N_;
  Ntarg=Ntarg_;penN=penN_;
...
BoseHubbardHamiltonian::BoseHubbardHamiltonian(int L_,int N_,double* tis_,
					       double* Uis_,int Ntarg_,double penN_,
					       double* Vis_):
  hamil(L_){
  L=L_;N=N_;Ntarg=Ntarg_;penN=penN_;
...
BoseHubbardHamiltonian::BoseHubbardHamiltonian(int L_,int N_,const std::vector<double>& tis_,
  ...
  hamil(L_),L(L_),N(N_),tis(tis_),Uis(Uis_),Vis(Vis_){
  Ntarg=Ntarg_;penN=penN_;
...
void BoseHubbardHamiltonian::initZ(){
  ...
  // op2 -> b+
  // op3 -> b
  // op4 -> n (for trap and penalty term)
  // op5 -> if penalty, then n^2 too
  int nrOps=6;
  ...
  for(int i1=0;i1<d;i1++){
    ...
    Z.setElement(ONE_c*(i1*i1),Indices(5,i1,i1)); // diagonal N^2
  }
...
void BoseHubbardHamiltonian::initHMPO(){
  ...
  for(int k=0;k<L;k++){
    ...
    if(k!=L-1){
      ...
      C.setElement(2*penN*ONE_c,Indices(0,3,4)); // first part of ni nj
    }
    if(k!=0&&k!=L-1)
      C.setElement(ONE_c,Indices(3,3,0));
    C.setElement(ONE_c*U_,Indices(0,Dr-1,1)); 
    C.setElement(ONE_c*(V_-2*Ntarg*penN),Indices(0,Dr-1,4));
    C.setElement(ONE_c*(penN),Indices(0,Dr-1,5)); // ni^2
    C.setElement(ONE_c*(penN*Ntarg*Ntarg/L),Indices(0,Dr-1,0)); // constant: could leave out
    if(k!=0){
      C.setElement(I_c*sqrt(t_prev),Indices(1,Dr-1,3));
      C.setElement(I_c*sqrt(t_prev),Indices(2,Dr-1,2));
      C.setElement(ONE_c,Indices(3,Dr-1,4)); // second part of ni nj
...
void BoseHubbardHamiltonian::computeTwoBodyTerm(mwArray& result,int pos) 
  ...
  if(tis.size()>1){
    ...
    t_=tis[0];U1_=U2_=Uis[0];V1_=V2_=Vis[0];
  }
  int d=N+1;
  ...
  if(firstCall){
    ...
    // left and right one-body term with V
  ...
  result=-t_*twobody+factorL*U1_*onebodyU1+factorR*U2_*onebodyU2
    +factorL*(V1_)*onebodymu1+factorR*(V2_)*onebodymu2;
...
void BoseHubbardHamiltonian::getExponentialMPO(MPO& expH,complex_t delta,bool even){
  ...
  for(int k=kfirst;k<klast;k+=2){
	...
	Vis[k]!=Vis[k-2]||
	Uis[k+1]!=Uis[k-1]||Vis[k+1]!=Vis[k-1]));
